...
// building block also fill string_params
...
// function c' x + f a wrapper around count_each_structure_type returns the
// energy function

double get_feature_counts_quadratic(char *sequence, char *structure,
                                    char *restricted, double **quadratic,
                                    double *linear, double &f);
// This function computes the P matrix (quadratic), the c vector (linear) and
// the f value
//    of a quadratic energy function x'Px + c'x + f
// just a dummy function (we don't have quadratic energy model in MultiRNAFold,
// equivalent with get_feature_counts Used to test
// create_structural_constraints_simfold.cpp for the quadratic case
...
// from a different file assumes the fm363 model
...
int generate_structure_withbb(char *sequence, char *known_structure,
                              char *restricted, char *turner_structure,
...
// original Vienna RNA file IT LOOKS LIKE THERE IS A MISTAKE int the Vienna RNA
// original file in the dangle sections!!! written on Nov 19, 2007
