...
// The constructor
{
  ...
  // this array holds V(i,j), and what (i,j) encloses: hairpin loop, stack pair,
  // internal loop or multi-loop
  nodes = new free_energy_node[total_length];
  if (nodes == NULL)
    giveup("Cannot allocate memory", "s_energy_matrix");
}
...
// compute the V(i,j) value
{
  ...
  {
    // compute free energy of hairpin loop, stack pair, internal loop and
    // multi-loop
...
void s_energy_matrix::compute_energy_restricted(int i, int j,
                                                str_features *fres)
// compute the V(i,j) value, if the structure must be restricted
{
  ...
  if (can_pair(sequence[i], sequence[j])) {
    if (fres[i].pair == i + 1)
      ...
      // there was a stupid bug here, I was calling H->compute_energy instead of
      // the restricted version. Fixed on June 30, 2007.
      min_en[1] = S->compute_energy(i, j);
...
void s_energy_matrix::compute_energy_sub_restricted(int i, int j,
                                                    str_features *fres)
// compute the V(i,j) value - suboptimals and restricted
{
  ...
  if (can_pair(sequence[i], sequence[j])) {
    ...
    min_en[1] = S->compute_energy(i, j);
    min_en[2] = VBI->compute_energy_restricted(i, j, fres);
    // I don't need restricted for VM_sub because I include dangling ends all
    // the time
    min_en[3] = VM_sub->compute_energy(i, j);
  }
