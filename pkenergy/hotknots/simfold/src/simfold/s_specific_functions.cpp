...
PARAMTYPE s_dangling_energy(int *sequence, char *structure, int i1, int i2,
                            int i3, int i4)
//      (   )...(   )
//      i1..i2..i3..i4
...
PARAMTYPE s_dangling_energy_left(int *sequence, char *structure, int i1, int i2,
                                 int i3, int i4)
//      (....(    )   )
//      i1   i3  i4  i2
...
PARAMTYPE s_dangling_energy_right(int *sequence, char *structure, int i1,
...
// POST: return dangling enthalpy between i2 and i3
{
  ...
  d_top =
      MIN(0, enthalpy_dangle_top[sequence[i2]][sequence[i1]][sequence[i2 + 1]]);
  d_bot =
      MIN(0, enthalpy_dangle_bot[sequence[i4]][sequence[i3]][sequence[i3 - 1]]);
...
PARAMTYPE s_dangling_enthalpy_left(int *sequence, int i1, int i2, int i3,
                                   int i4)
//      (....(    )   )
//      i1   i3  i4  i2
// PRE:  (i1, i2) and (i3, i4) are pairs, i1 and i3 are neighbours, i3 < i2
// POST: return dangling enthalpy between i1 and i3
{
  ...
  d_top =
      MIN(0, enthalpy_dangle_top[sequence[i1]][sequence[i2]][sequence[i1 + 1]]);
...
PARAMTYPE s_dangling_enthalpy_right(int *sequence, int i1, int i2, int i3,
                                    int i4)
//      (    (    )...)
//      i1   i3  i4  i2
// PRE:  (i1, i2) and (i3, i4) are pairs, i1 and i3 are neighbours, i3 < i2
// POST: return dangling enthalpy between i4 and i2
{
  ...
  d_top =
      MIN(0, enthalpy_dangle_top[sequence[i4]][sequence[i3]][sequence[i4 + 1]]);
  d_bot =
      MIN(0, enthalpy_dangle_bot[sequence[i1]][sequence[i2]][sequence[i2 - 1]]);
...
double free_energy_simfold_restricted(char *sequence, char *structure,
                                      char *restricted)
// PRE:  sequence and structure are given as input
// POST: returns the free energy of sequence folded into structure
//       Worst case complexity = n^2 (n = length of sequence)
...
PARAMTYPE s_calculate_energy(int *sequence, char *csequence, char *structure,
                             str_features *f, str_features *fres)
...
// restricted to be used for hairpin loops only
{
  ...
  for (i = 0; i < nb_nucleotides; i++) {
    ...
    if (f[i].pair == -1 && !cannot_add_dangling[i]) {
      ...
      // ).(
      {
        if (no_dangling_ends)
          ...
          dang = MIN(0, s_dangling_energy(sequence, structure, f[i - 1].pair,
                                          i - 1, i + 1, f[i + 1].pair));
        AUpen = AU_penalty(sequence[i + 1], sequence[f[i + 1].pair]);
        if (debug) {
          printf("%d - dangle1 \t- add energy %6d\n", i, dang);
    ...
    if (f[i].type == STACK) {
      ...
      // Mar 24, 2008: I don't think I need this, it's dealt with inside
      // s_hairpin_loop
      if (fres != NULL)
      ...
      en = s_internal_loop::get_energy(i, f[i].pair, ip, jp, sequence,
                                       ptable_restricted);
      if (debug)
...
// to check!!!
{
  ...
  for (i = 0; i < nb_nucleotides; i++) {
    ...
    if (f[i].pair == -1 && !cannot_add_dangling[i]) {
      ...
      // .( or ..(
      {
        dang = MIN(0, enthalpy_dangle_bot[sequence[f[i + 1].pair]]
      ...
      {
        dang =
            MIN(0, enthalpy_dangle_top[sequence[i - 1]][sequence[f[i - 1].pair]]
                                      [sequence[i]]);
        if (debug)
          printf("%d - dangle2 \t- add energy %6d\n", i, dang);
        energy += dang;
      ...
      // ).(
      {
        dang = MIN(0, s_dangling_enthalpy(sequence, f[i - 1].pair, i - 1, i + 1,
                                          f[i + 1].pair));
        AUpen = AU_penalty_enthalpy(sequence[i + 1], sequence[f[i + 1].pair]);
        if (debug) {
          printf("%d - dangle1 \t- add energy %6d\n", i, dang);
    ...
    } else // (f[i].type == MULTI)
    {
      ...
      misc_energy +=
          enthalpy_misc.multi_helix_penalty * (f[i].num_branches + 1);

      // add AU_penalties for multi-loop
      AUpen += AU_penalty_enthalpy(sequence[i], sequence[f[i].pair]);
      for (h = 0; h < f[i].num_branches; h++)
        AUpen += AU_penalty_enthalpy(sequence[f[i].bri[h]],
                                     sequence[f[f[i].bri[h]].pair]);

      // add dangling energies for multi-loop
      dang += s_dangling_enthalpy_left(sequence, i, f[i].pair, f[i].bri[0],
                                       f[f[i].bri[0]].pair);
      for (l = 0; l < f[i].num_branches - 1; l++)
        dang += s_dangling_enthalpy(sequence, f[i].bri[l], f[f[i].bri[l]].pair,
                                    f[i].bri[l + 1], f[f[i].bri[l + 1]].pair);
      dang += s_dangling_enthalpy_right(
...
// POST: fold sequence, return the MFE structure in structure, and return the
// MFE
{
  double min_energy;
...
// MFE
{
  ...
  for (int i = 0; i < strlen(sequence); i++) {
    if ((restricted[i] == '(' || restricted[i] == ')' ||
      ...
      printf("ERROR!!! There is something wrong with the structure, doesn't "
             "match restricted\n");
  ...
  double correct_energy =
      free_energy_simfold_restricted(sequence, structure, restricted);
  if (fabs(correct_energy - min_energy) > 1.0) {
    printf("ERROR!!! The dp energy is different from the energy calculated at "
           "the end!!\n");
    printf("%s\n%s\n%s\n correct_energy=%.2lf, energy=%.2lf\n", sequence,
           restricted, structure, correct_energy, min_energy);
    printf("ERROR!!! The dp energy is different from the energy calculated at "
           "the end!!\n");
    exit(1);
  }
...
int simfold_unordered_suboptimals_range(char *sequence, double max_energy,
                                        char structures[][MAXSLEN],
                                        double energies[], int num_subopt)
// Compute all suboptimal structures (limited by MAXSUBSTR), which have free
// energy <= max_energy
//     the free energies include the 5' and 3' dangling energies in all cases
{
  ...
  if (min_energy >= max_energy) {
    ...
    // printf ("R1: %s %d\nR2: %s %d\n\n", sequence1, strlen(sequence1),
    // sequence2, strlen(sequence2));
...
int simfold_restricted_unordered_suboptimals_range(char *sequence,
                                                   char *restricted,
                                                   double max_energy,
                                                   char structures[][MAXSLEN],
                                                   double energies[])
// Compute all (restricted) suboptimal structures (limited by MAXSUBSTR), which
// have free energy <= max_energy
//     the free energies include the 5' and 3' dangling energies in all cases
{
  int actual_num_str;
  char structure[MAXSLEN];
  ...
  if (min_energy >= max_energy) {
    ...
    s_sub_folding *sub_fold =
        new s_sub_folding(sequence, restricted, energy_range);
...
int simfold_unordered_suboptimals(char *sequence, int number,
                                  char structures[][MAXSLEN], double energies[])
// compute "number" suboptimal structures, with dangling energies added
// everywhere return the number of suboptimal structures (between 1 and number)
{
  int actual_num_str;
  char structure[MAXSLEN];
  double min_energy, enthalpy;
...
int simfold_ordered_suboptimals(char *sequence, int number,
                                char structures[][MAXSLEN], double energies[])
// compute "number" suboptimal structures, with dangling energies added
// everywhere in fact, compute 2*number structures, then compute the good free
// energy, reorder, and get the best "number" structures return the number of
// suboptimal structures (between 1 and number)
{
  int actual_num_str;
  char structure[MAXSLEN];
  double min_energy, enthalpy;
  ...
  if (number == 1) {
    ...
    // added on Oct 16, 2007. The good free energies have to be recomputed here!
    ...
    if (actual_num_str > number)
      actual_num_str = number;
    for (i = 0; i < actual_num_str; i++) {
      // printf ("%2d  %s    %.2lf \n =======\n", i+1,
      // tmp_structures[positions[i]], tmp_energies[positions[i]]);
...
int simfold_restricted_unordered_suboptimals(char *sequence, char *restricted,
                                             int number,
                                             char structures[][MAXSLEN],
                                             double energies[]) {
  int actual_num_str;
...
int simfold_restricted_all_suboptimals(char *sequence, char *restricted,
                                       char structures[][MAXSLEN],
                                       double energies[]) {
  int actual_num_str;
  char structure[MAXSLEN];
...
int simfold_restricted_all_mfe_structures(char *sequence, char *restricted,
                                          char structures[][MAXSLEN],
                                          double energies[])
// return MAXSUBSTR suboptimal structures, which have free energy <= 0. Note
// that the suboptimal structure computation adds both 5' and 3' dangling ends
// in any case. The suboptimal structures are not reordered in this function.
{
  int actual_num_str;
  char structure[MAXSLEN];
...
PFTYPE simfold_partition_function_smart(char *sequence, int ignore_dangles,
                                        int compute_gradient_dangles)
// unrestricted for now
{
  PFTYPE pf;
  int i, j;
  s_partition_function *part = new s_partition_function(
      sequence, ignore_dangles, compute_gradient_dangles);
...
void simfold_gradient_smart(char *sequence, PFTYPE *grad, int ignore_dangles,
                            int compute_gradient_dangles)
// unrestricted for now
{
  PFTYPE pf;
...
PFTYPE simfold_f_and_gradient_smart(char *sequence, char *restricted,
                                    PFTYPE *grad, int ignore_dangles,
                                    int compute_gradient_dangles)
// unrestricted for now
{
  PFTYPE pf;
  int i, j;
  s_partition_function *part = new s_partition_function(
      sequence, ignore_dangles, compute_gradient_dangles, restricted);
          ...
          if (can_pair(nuc_to_int(sequence[i]), nuc_to_int(sequence[j])))
          {
              ...
              for (k=0; k < actual_num_str; k++)
              {
                  if (has_base_pair(i,j, tmp_structures[k]))
                  {
                      ...
                      for (l=0; l < k; l++)
                      {
                          if (identical_structure (i, j, tmp_structures[l],
  tmp_structures[k])) identical = 1;
                      }
                      if (!identical)
                          up += exp_free_energy_partial (i, j, sequence,
  tmp_structures[k]);
                      //else
                      //    printf ("Str %s identical with other for i=%d,
...
// for now, the following functions exist only if the parameters are [long]
// double

#ifdef INCLUDE_FUNCTIONS
#include "s_partition_function_complex.h"

PFTYPE simfold_partition_function_smart_numerical(char *sequence,
                                                  int ignore_dangles,
                                                  int compute_gradient_dangles)
// unrestricted for now
{
  Complex pf;
  int i, j;
  s_partition_function_complex *part = new s_partition_function_complex(
      sequence, ignore_dangles, compute_gradient_dangles);
  pf = part->compute_partition_function();
  // part->print_u();
  // part->compute_base_pair_probabilities();
...
PFTYPE simfold_partition_function_smart_double(char *sequence,
                                               int ignore_dangles,
                                               int compute_gradient_dangles)
// unrestricted for now
{
  ...
  int i, j;
  s_partition_function_complex *part = new s_partition_function_complex(
      sequence, ignore_dangles, compute_gradient_dangles, 1);
...
void simfold_gradient_numerical(char *sequence, PFTYPE *grad,
                                int ignore_dangles,
                                int compute_gradient_dangles)
// unrestricted for now
{
...
PFTYPE simfold_f_and_gradient_smart_double(char *sequence, PFTYPE *grad,
                                           int ignore_dangles,
                                           int compute_gradient_dangles)
// unrestricted for now
...
PFTYPE simfold_f_and_gradient_smart_numerical(char *sequence, PFTYPE *grad,
                                              int ignore_dangles,
                                              int compute_gradient_dangles,
                                              int which_param)
// unrestricted for now
// if which_param is -1, compute gradient for all params, otherwise just for
// which_param
{
  Complex pf;
  int i, j;
