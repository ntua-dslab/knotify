...
public:
  ...
  // compute the multi-loop branch which is right at the left of j, the
  // restricted case

  void compute_energy_FM_restricted(int j, str_features *fres);
  // computes a partial multi-loop, the restricted case

  PARAMTYPE get_FM_energy(int i, int j) {
    if (i >= j)
      return INF;
    return FM[index[i] - i + j];
  }
  // return the energy of FM(i,j)

  PARAMTYPE get_FM1_energy(int i, int j) {
    if (i >= j)
      return INF;
    return FM1[index[i] - i + j];
  }
...
protected:
  ...
  PARAMTYPE *FM;  // 2D array to keep the energy for FM (actually 1D of length
                  // n*(n-1)/2)
  PARAMTYPE *FM1; // 2d array to keep the energy for FM1 (actually 1D of length
                  // n*(n-1)/2) FM1 contains the rightmost multi-loop branch at
                  // this point
};
