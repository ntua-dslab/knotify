...
s_partition_function::s_partition_function(char *cseq, int ignore,
                                           int compute_g_dangles, char *res)
// The constructor
{
  ...
  IFD no_dangling_ends = 1; // not used
  else simple_dangling_ends = 1;

  csequence = cseq; // just refer it from where it is in memory
  restricted = res;

  if (restricted != NULL) {
    if ((ptable_restricted = new int[seqlen]) == NULL)
      giveup("Cannot allocate memory", "s_partition_function");
  ...
  if (GlogZexhaustive == NULL)
    giveup("Cannot allocate memory", "s_partition_function");

  GlogZ_finite_differences = new PFTYPE[num_params];
  if (GlogZ_finite_differences == NULL)
    giveup("Cannot allocate memory", "s_partition_function");
...
void s_partition_function::initialize_arrays() {
  ...
  for (i = 0; i < seqlen; i++) {
    ...
    // for (j = i; j <= MIN(i + TURN, seqlen-1); j++)
    {
      ...
      IFD {
        u[ij] = 1;
        if (exists_restricted_ptable(i - 1, j + 1, ptable_restricted))
          u[ij] = 0.0;
        u1[ij] = 0;
        s1[ij] = 0;
        s2[ij] = 0;
...
// the recursions are taken from Ding and Lawrence, "A statistical sampling
// algorithm for RNA secondary structure prediction", NAR 2003
{
  int i, j, ij;
  ...
  for (j = 1; j < seqlen; j++) {
    ...
    for (i = j - 1; i >= 0; i--) {
      ...
      IFD {
        compute_upm_nodangles(
            i, j); // doesn't matter where it is, all dependencies have been
                   // computed at previous steps
        ...
        compute_upm(i, j); // doesn't matter where it is, all dependencies have
                           // been computed at previous steps
        compute_up(i, j);  // must be after upm
        ...
        // printf ("u[%d,%d] = %g\n", i, j, u_ip_jp[ij] + u_ip_ju[ij] +
        // u_iu_jp[ij] + u_iu_ju[ij]);
  ...
  IFD { Z = u[firstlast]; }
    ...
    Z = u_ip_jp[firstlast] + u_ip_ju[firstlast] + u_iu_jp[firstlast] +
        u_iu_ju[firstlast];
  }
  return Z;
}
...
// dangle_bot
{
  ...
  if (dangle_bot[sequence[i]][sequence[j]][sequence[k]] == INF)
    return 1.0;
  // try calling the arrays directly, it's actually a looot faster (about 5
  // times on length 200)
...
// called only when no dangling ends
{
  ...
  // we don't need separate cases to compute u, because we don't care about
  // dangling ends

  if (restricted != NULL) {
    u[ij] = 1.0;
    // make sure u[ij] can't contain only unpaired bases if something between i
    // and j must be paired
    if (exists_restricted_ptable(i - 1, j + 1, ptable_restricted))
      u[ij] = 0.0;
    if (ptable_restricted[i] > j ||
        (ptable_restricted[j] > -1 && ptable_restricted[j] < i)) {
      u[ij] = 0.0;
      return;
    }
...
// called only when no dangling ends
{
  // if (h < 1)  return;  // we don't want this here, because we don't care
  // about dangling ends
  ...
  // TODO: to check!
  // l should go to j, it's important for the restricted case. Not sure if we
  // need to go only to j-2 for the unrestricted case, this is the only
...
// s3 doesn't contain the helix penalty
{
  ...
  for (l = h + 1; l <= j; l++) {
    ...
    if (exists_restricted_ptable(l, j, ptable_restricted))
      exp_free = 0.0;
    else
      exp_free = EXPC[j - l];
...
// i and j close a multi-loop
{
  ...
  /*
  // I don't think we need all these cases, since we don't care about dangling
  ends
  // there must be at least one more branch (i.e. 5 nucleotides) from l+1 to
  j-1: l+1+TURN <= j-1 for (l=i+2; l < j-TURN-2; l++)    // case ((...)--(--)-)
  {
  ...
  */
  ...
  if (restricted != NULL) {
    ...
    // for (h=i+3; h < j-TURN-2; h++)    // case (....(...)--(--)-)
    {
      ...
      if (ptable_restricted[h] > -1)
        break; // that's it, don't go further because this h must be paired
    }
  } else // I don't think I need a separate branch here, but anyway, I leave it
         // for now
  {
    for (h = i + 1; h < j - TURN - 2; h++)
    // for (h=i+3; h < j-TURN-2; h++)    // case (....(...)--(--)-)
...
// modified from Ding and Lawrence, to be able to add the right most d5 dangling
// end: (--(...)..) has at least 2 branches, and h is paired with some l in
// between h and j
{
  int hj, hl, l, lp2j, lp1j;
  hj = index[h] + j - h;
...
// j paired or j-1 paired
{
  ...
  u_ip_jp[ij] += up[ijm1] * exp_AUpenalty(i, j - 1) *
                 exp_dangle3(j - 1, i, j); // case (...).
  ...
  for (l = i + 1; l < j - 2; l++) // case (...)-(--)
  {
    ...
    lp1j = index[l + 1] + j - l - 1;
    u_ip_jp[ij] += up[il] * exp_AUpenalty(i, l) *
                   (u_ip_jp[lp1j] +
                    exp_dangle3(l, i, l + 1) * (u_ip_jp[lp2j] + u_iu_jp[lp2j]));
    // if (u_ip_jp[lp2j] != 1)
    //    u_ip_jp[ij] += up[il] * exp_AUpenalty (i, l) * exp_dangle3 (l, i, l+1)
    //    * u_ip_jp[lp2j];
    // if (u_iu_jp[lp2j] != 1)
    //    u_ip_jp[ij] += up[il] * exp_AUpenalty (i, l) * exp_dangle3 (l, i, l+1)
    //    * u_iu_jp[lp2j];
...
// j must be unpaired and j-1 must be unpaired
{
  ...
  for (l = i + 1; l < j - 2; l++) // case (...)...---
  {
    ...
    lp1j = index[l + 1] + j - l - 1;
    u_ip_ju[ij] +=
        up[il] * exp_AUpenalty(i, l) *
        (u_ip_ju[lp1j] +
         exp_dangle3(l, i, l + 1) *
             (u_ip_ju[lp2j] + u_iu_ju[lp2j])); // put them together to be faster
  }
}
...
// j paired or j-1 paired
{
  ...
  // u_iu_jp[ij] += up[hjm1] * exp_dangle5 (j-1, h, h-1) * exp_AUpenalty (h,
  // j-1) * exp_dangle3(j-1,h,j);
  //}
...
// j paired or j-1 paired
{
  ...
  // case .(...).
  s1_jp[hj] += up[hjm1] * exp_dangle5(j - 1, h, h - 1) *
               exp_AUpenalty(h, j - 1) * exp_dangle3(j - 1, h, j);
  ...
  for (l = h + 1; l < j - 2; l++) // .(...)---
  {
    ...
    s1_jp[hj] += up[hl] * exp_dangle5(l, h, h - 1) * exp_AUpenalty(h, l) *
                 (u_ip_jp[lp1j] +
                  exp_dangle3(l, h, l + 1) * (u_ip_jp[lp2j] + u_iu_jp[lp2j]));
  }
}
...
// j unpaired and j-1 unpaired
{
  ...
  for (l = h + 1; l < j - 2; l++) // .(...)---
  {
    ...
    s1_ju[hj] += up[hl] * exp_dangle5(l, h, h - 1) * exp_AUpenalty(h, l) *
                 (u_ip_ju[lp1j] +
                  exp_dangle3(l, h, l + 1) * (u_ip_ju[lp2j] + u_iu_ju[lp2j]));
  }
}
...
void s_partition_function::compute_up(int i, int j) {
  ...
  if (restricted != NULL) {
    ...
    if (ptable_restricted[i] == -1 || ptable_restricted[j] == -1)
      return;
    // i is supposed to pair with someone else
    if (ptable_restricted[i] > -1 && ptable_restricted[i] != j)
      return;
    // j is supposed to pair with someone else
    if (ptable_restricted[j] > -1 && ptable_restricted[j] != i)
      return;
    // make sure we disallow base pairs crossing the restricted base pairs
    for (int k = i + 1; k < j; k++) {
      if (ptable_restricted[k] > -1 &&
          (ptable_restricted[k] < i || ptable_restricted[k] > j))
        return;
    }
  }
  ...
  // hairpin loop
  en_hairpin =
      s_hairpin_loop::get_energy(i, j, sequence, csequence, ptable_restricted);
  ...
  /*
  // weirdly, if this has the GGG hairpin, it's added here, although it
  shouldn't
  // so try to remove it in case it's added
  //    that would only give me equality between the exhaustive and the true u's
  and up's if (j-i-1 == 3 && i > 1)
  {
    ...
    for (ip = i + 1; ip <= MIN(j - 2, i + MAXLOOP + 1); ip++) {
      ...
      for (jp = minq; jp < j; jp++) {
        if (sequence[ip] + sequence[jp] == 3 ||
          ...
          if (ip == i + 1 && jp == j - 1)
            continue; // we don't want stacked pairs here
          ...
          if (en_internal >= INF / 2) {
            ...
            num_internal_in_up++;
            //                     if (i==0 && j==8)    printf ("r3, ip=%d,
            //                     jp=%d, up[0,8] = %g, en_internal=%d\n", ip,
            //                     jp, up[ij], en_internal);
          }
        }
      }
    }
...
// this assumes we include dangling ends. For the no dangling end version, see
// compute_upm_nodangles i and j close a multi-loop
{
  int ij = index[i] + j - i;
  int l, h;
  int ip1l, ip2l, lp2jm1, lp1jm1, lp2jm2, lp1jm2, lp2jm3, lp1jm3, hj;
        ...
        (u1_ip_jp[lp1jm1] // [(...)(.-..)] or [(...)(.-..).]
         ...
         exp_dangle3(l, i + 1, l + 1) * EXPC[1] *
             (u1_ip_jp[lp2jm1] +
              u1_iu_jp[lp2jm1] + // [(...).-(...)] or [(...).-(...).]
              exp_dangle5(i, j, j - 1) *
                  (u1_ip_ju[lp2jm1] + u1_iu_ju[lp2jm1])) + // [(...).-(...)-..]
  ...
  for (l = i + 3; l < j - TURN - 2; l++) // case [.(...)--(--)-]
  {
    ...
    upm[ij] +=
        up[ip2l] * EXPC[1] * exp_dangle3(i, j, i + 1) *
        exp_AUpenalty(i + 2, l) *
        (u1_ip_jp[lp1jm1] // [.(...)(.-..)] or [.(...)(.-..).]
         +
...
// between h and j
{
  ...
  for (l = h + 1; l < j - 3; l++) // .(...)
  {
    ...
    s2_jp[hj] += up[hl] * exp_dangle5(l, h, h - 1) * exp_AUpenalty(h, l) *
                 (u1_ip_jp[lp1j] + exp_dangle3(l, h, l + 1) * EXPC[1] *
                                       (u1_ip_jp[lp2j] + u1_iu_jp[lp2j]));
  }
}
...
// between h and j
{
  ...
  for (l = h + 1; l < j - 3; l++) // .(...)
  {
    ...
    s2_ju[hj] += up[hl] * exp_dangle5(l, h, h - 1) * exp_AUpenalty(h, l) *
                 (u1_ip_ju[lp1j] + exp_dangle3(l, h, l + 1) * EXPC[1] *
                                       (u1_ip_ju[lp2j] + u1_iu_ju[lp2j]));
  }
}
...
// j must be paired, or j-1 must be paired
{
  ...
  for (l = j - 1; l <= j; l++) // (...) or (...).
  {
    il = index[i] + l - i;
    u1_ip_jp[ij] +=
        up[il] * exp_AUpenalty(i, l) * fd3(j + 1, i, l) * EXPC[j - l + 1];
  }
  ...
  for (l = i + 1; l < j - 2; l++) // (...)-(---) or (...)-(---).
  {
    ...
    u1_ip_jp[ij] += up[il] * exp_AUpenalty(i, l) *
                    (u1_ip_jp[lp1j] + exp_dangle3(l, i, l + 1) * EXPC[1] *
                                          (u1_ip_jp[lp2j] + u1_iu_jp[lp2j]));
  }
  u1_ip_jp[ij] *= EXPB1;
}
...
// j must be paired, or j-1 must be paired
{
  ...
  int ijm1 = index[i] + j - 1 - i;
  u1_ip_ju_jm1p[ij] +=
      up[ijm1] * exp_AUpenalty(i, j - 1) * fd3(j + 1, i, j - 1) * EXPC[1];
  ...
  for (l = i + 1; l < j - 2; l++) // (...)-(---).
  {
    ...
    u1_ip_ju_jm1p[ij] +=
        up[il] * exp_AUpenalty(i, l) *
        (u1_ip_ju_jm1p[lp1j] + exp_dangle3(l, i, l + 1) * EXPC[1] *
                                   (u1_ip_ju_jm1p[lp2j] + u1_iu_ju_jm1p[lp2j]));
  }
...
// j must be unpaired, and j-1 must be unpaired
{
  ...
  for (l = i + 1; l <= j - 2; l++) // (...)----
  {
    ...
    if (l + 2 < j) // (...)-(--)-
    {
      ...
      lp2j = index[l + 2] + j - l - 2;
      u1_ip_ju[ij] +=
          temp * (u1_ip_ju[lp1j] + exp_dangle3(l, i, l + 1) * EXPC[1] *
                                       (u1_ip_ju[lp2j] + u1_iu_ju[lp2j]));
    }
  }
  u1_ip_ju[ij] *= EXPB1;
...
// j must be paired, or j-1 must be paired
{
  ...
  {
    ip1l = index[i + 1] + l - i - 1;
    u1_iu_jp[ij] += up[ip1l] * exp_AUpenalty(i + 1, l) *
                    exp_dangle5(l, i + 1, i) * // EXPC[1] * (added it to the end
  ...
  for (l = i + 2; l < j - 2; l++) // .(...)-(---) or .(...)-(---).
  {
    ...
    lp2j = index[l + 2] + j - l - 2;
    u1_iu_jp[ij] += up[ip1l] * exp_AUpenalty(i + 1, l) *
                    exp_dangle5(l, i + 1, i) * EXPC[1] *
                    (u1_ip_jp[lp1j] + exp_dangle3(l, i + 1, l + 1) * EXPC[1] *
                                          (u1_ip_jp[lp2j] + u1_iu_jp[lp2j]));
  }
...
// j must be paired, or j-1 must be paired
{
  ...
  int ip1jm1 = index[i + 1] + j - 1 - i - 1; // .(...).
  u1_iu_ju_jm1p[ij] += up[ip1jm1] * exp_AUpenalty(i + 1, j - 1) *
                       exp_dangle5(j - 1, i + 1, i) * fd3(j + 1, i + 1, j - 1) *
                       EXPC[2];
  ...
  for (l = i + 2; l < j - 2; l++) // .(...)-(---) or .(...)-(---).
  {
    ...
    lp2j = index[l + 2] + j - l - 2;
    u1_iu_ju_jm1p[ij] +=
        up[ip1l] * exp_AUpenalty(i + 1, l) * exp_dangle5(l, i + 1, i) *
        EXPC[1] *
        (u1_ip_ju_jm1p[lp1j] + exp_dangle3(l, i + 1, l + 1) * EXPC[1] *
                                   (u1_ip_ju_jm1p[lp2j] + u1_iu_ju_jm1p[lp2j]));
  }
...
// j must be unpaired, and j-1 must be unpaired
{
  ...
  for (l = i + 2; l < j - 2; l++) // .(...)-(--)-..
  {
    ...
    lp1j = index[l + 1] + j - l - 1;
    lp2j = index[l + 2] + j - l - 2;
    u1_iu_ju[ij] += up[ip1l] * exp_AUpenalty(i + 1, l) *
                    exp_dangle5(l, i + 1, i) * EXPC[1] *
                    (u1_ip_ju[lp1j] + exp_dangle3(l, i + 1, l + 1) * EXPC[1] *
                                          (u1_ip_ju[lp2j] + u1_iu_ju[lp2j]));
  }
...
PFTYPE s_partition_function::fd3(int jplus1, int h, int l) {
  if (l > jplus1 - 1)
    giveup("Error, l > j", "f function, partition_function");
  if (l == jplus1 - 1)
    return 1.0;
...
// j paired or j-1 paired
{
  ...
  int hjm1 = index[h] + j - 1 - h;
  s3_ju_jm1p[hj] += up[hjm1] * exp_dangle5(j - 1, h, h - 1) *
                    exp_AUpenalty(h, j - 1) * fd3(j + 1, h, j - 1) * EXPC[1];
  ...
  for (l = h + 1; l < j - 2; l++) {
    ...
    hl = index[h] + l - h;
    s3_ju_jm1p[hj] +=
        up[hl] * exp_dangle5(l, h, h - 1) * exp_AUpenalty(h, l) *
        (u1_ip_ju_jm1p[lp1j] + exp_dangle3(l, h, l + 1) * EXPC[1] *
                                   (u1_ip_ju_jm1p[lp2j] + u1_iu_ju_jm1p[lp2j]));
  }
}
...
// PRE: the partition function arrays have been filled
{
  int h, l;
  for (h = 0; h < seqlen; h++) {
    // Removed TURN on Apr 27, 2008. With the TURN it doesn't work for the
    // restricted case
    ...
    // for (l=seqlen-1; l > h+TURN; l--)
    {
      ...
      IFD {
        ...
        // the next 4 are only needed to compute the partial derivative wrt
        // multi_free_base_penalty moved to the gradient
...
// prints all the base pair probabilities above the given threshold
{
  int i, j;
  printf(
      "Base pair probabilities above threshold %.2lf\nIndeces start from 0.\n",
      threshold);
...
void s_partition_function::compute_p(int h, int l) {
  ...
  IFD {
    ...
    if (h > TURN) {
      term1 += (u_ip_jp[h - 1] + u_iu_jp[h - 1] +
                exp_dangle5(l, h, h - 1) * (u_ip_ju[h - 1] + u_iu_ju[h - 1]));
    } else if (h > 0) {
  ...
  IFD {
    if (l < seqlen - 1)
      term2 = u[index[l + 1] + seqlen - 1 - (l + 1)];
    else
      term2 = 1;
  }
  else {
    if (l < seqlen - 3) {
      ...
      term2 += (u_ip_jp[lp1n] + u_ip_ju[lp1n] +
                exp_dangle3(l, h, l + 1) * (u_ip_jp[lp2n] + u_ip_ju[lp2n] +
                                            u_iu_jp[lp2n] + u_iu_ju[lp2n]));
    } else if (l < seqlen - 1) {
  ...
  IFD {
    ...
    p[hl] = term1 * up[hl] * term2 * exp_AUpenalty(h, l) /
            (u_ip_jp[zeronminus1] + u_ip_ju[zeronminus1] +
             u_iu_jp[zeronminus1] + u_iu_ju[zeronminus1]);
  }
  ...
  if (!ignore_internal) {
    ...
    for (i = MAX(0, h - MAXLOOP - 1); i < h; i++) {
      // Nov 19, 2007: FIXED a very stupid bug. It was -2 below when it should
      // be +2. That screwed up the calculation of the gradient values.
      for (j = l + 1; j <= MIN(MAXLOOP + l - h + i + 2, seqlen - 1); j++) {
        if (sequence[i] + sequence[j] == 3 || sequence[i] + sequence[j] == 5) {
          if (i == h - 1 && j == l + 1)
            continue; // we don't want stacked pairs here
          int ij = index[i] + j - i;
          if (up[ij] == 0)
  ...
  for (i = 0; i < h; i++) {
    ...
    IFD pml += EXPC[h - i - 1] * pm[il];
    else {
      if (i < h - 1) // we must add d3 (which is added in pmd3); also add d5
                     // here, if i < h-2
      {
        pml += EXPC[h - i - 1] * (i < h - 2 ? exp_dangle5(l, h, h - 1) : 1) *
               (pmd3_noneedmidd3[il] +
                pmd3_needmidd3[il] * exp_dangle3(l, h, l + 1) * EXPC[1]);
        // (.-[...](---)-)    i.-h...l(---)-j
        // (.-[...].-(---)-)    i.-h...l.-(---)-j
      } else // case ((... , no dangling end
      {
        pml += pmnod3_noneedmidd3[il] +
      ...
      IFD {
                  ...
                  (                     // first, the case ((..-)-[...])
                      ...
                      + exp_dangle3(i, l + 1, i + 1) * EXPC[1] *
                            (u1_ip_jp[ip2hm1] + u1_iu_jp[ip2hm1] +
                             exp_dangle5(l, h, h - 1) *
                                 (u1_ip_ju[ip2hm1] + u1_iu_ju[ip2hm1]))
                      // (..-(..-).-[...])
                  );
        }
        ...
        pml +=
            ...
            (term1 +
             ...
             // case ((..-)-[...].-)
             pm1nod3_needendd3[il] *
                 exp_dangle3(l, h, l + 1) * // EXPC added in pm1nod3
                 (u1_ip_jp[ip1hm1] +
             ...
             // case (.(..-)-[...].-)
             + pm1d3_needendd3[il] *
                   exp_dangle3(l, h, l + 1) * // don't need EXPC[1]
                   (u1_ip_jp[ip2hm1] +
                    exp_dangle5(l, h, h - 1) * u1_ip_ju[ip2hm1])
             // (.(...).-[...].-)        i.(...)h...l.-j
             ...
             + pm1d3_needendd3[il] *
                   exp_dangle3(l, h, l + 1) * // don't need EXPC[1]
                   (u1_iu_jp[ip2hm1] +
                    exp_dangle5(l, h, h - 1) * u1_iu_ju[ip2hm1])
             // (..-(...)..-[...].-)      i..-(...)h...l.-j
...
//  (.-(    )-(---)-)
{
  ...
  for (j = l + TURN + 3; j < seqlen; j++) {
    if (can_pair(sequence[i], sequence[j])) {
      ...
      lp2jm3 = index[l + 2] + j - 3 - l - 2;
      pmd3_needmidd3[il] +=
          exp_AUpenalty(i, j) * exp_dangle3(i, j, i + 1) * p[ij] /
          up[ij] * // don't need to add EXPC[1] here, it's added in the calling
                   // function
...
//  (.-(    )-(---)-)
{
  ...
  for (j = l + TURN + 3; j < seqlen; j++) {
    if (can_pair(sequence[i], sequence[j])) {
      ...
      pmd3_noneedmidd3[il] +=
          exp_AUpenalty(i, j) * exp_dangle3(i, j, i + 1) * p[ij] /
          up[ij] * // shouldn't add EXPC[1] here because it's in the calling
                   // function
...
//  ((    )-(---)-)
{
  ...
  for (j = l + TURN + 3; j < seqlen; j++) {
    if (can_pair(sequence[i], sequence[j])) {
      ...
      lp2jm3 = index[l + 2] + j - 3 - l - 2;
      pmnod3_needmidd3[il] +=
          exp_AUpenalty(i, j) * p[ij] /
          up[ij] * // no need to add EXPC[1] here, it's added in the calling
                   // function
          (u1_ip_jp[lp2jm1] + u1_iu_jp[lp2jm1] // ..))  or ..).)
           + exp_dangle5(i, j, j - 1) *
                 (u1_ip_ju[lp2jm1] + u1_iu_ju[lp2jm1])); //)-..)
...
//  region l+1  - j-1 has at least one branch
//  ih    l(---)-j
//  ((    )(---)-)    // right branch follows right after, no free base in
//  between
{
  int j;
  int ij, lp1jm1, lp1jm2, lp1jm3, lp2jm1, lp2jm2, lp2jm3, il;
...
//  ( (    )........)
{
  ...
  for (j = l + 2; j < seqlen; j++) {
    if (can_pair(sequence[i], sequence[j])) {
      ...
      if (up[ij] == 0)
        continue;
      pm1d3_needendd3[il] += exp_dangle3(i, j, i + 1) * EXPC[1] * p[ij] /
                             up[ij] * exp_AUpenalty(i, j) * EXPC[j - l - 1] *
                             (l + 2 < j ? exp_dangle5(i, j, j - 1) : 1);
    }
  }
}
...
// the 2 free bases are not aplicable here because we don't care about dangling
// ends i---------h...l.-.j
// (-(---)-..[...].-.)
{
  int i, ij;
...
//  region i+1  - h-3 has at least one branch, and there must at least 2 free
//  bases before h
...
//  region i+1  - h-1 has at least one branch, and there must be 0 or 1 free
//  bases left of h
// also add the dangling end d5(i,j,j-1)
// i------.h...l.-.j
// (-(---).[...].-.) or (-(---)[...].-.)
{
  ...
  for (i = 0; i < h - TURN - 2; i++) {
    if (can_pair(sequence[i], sequence[j])) {
      ...
      int ip1hm1 = index[i + 1] + h - 1 - (i + 1);
      int ip2hm1 = index[i + 2] + h - 1 - (i + 2);
      pm2d5_noneedmidd5[hj] +=
          p[ij] / up[ij] * exp_AUpenalty(i, j) * exp_dangle5(i, j, j - 1) *
          // first, ((---)[...].-.) or ((---).[...].-.)
          (u1_ip_jp[ip1hm1] + exp_dangle3(i, j, i + 1) * EXPC[1] *
                                  (u1_ip_jp[ip2hm1] + u1_iu_jp[ip2hm1]));
    }
  }
}
...
int s_partition_function::identical_structure(int i, int j, char *structure1,
                                              char *structure2)
...
PFTYPE s_partition_function::exp_free_energy_partial(int i, int j, char *seq,
                                                     char *structure,
                                                     int removeAU)
...
// return exp of free energy of sequence and structure, for region i-j
{
  ...
  for (k = i; k <= j; k++) {
    ...
    if (restricted != NULL)
      partial_restricted[k - i] = restricted[k];
  }
  partial_sequence[k - i] = '\0';
  partial_structure[k - i] = '\0';
  if (restricted != NULL)
    partial_restricted[k - i] = '\0';
  // printf ("Partial: %s %s\n", partial_sequence, partial_structure);
  if (restricted == NULL)
    energy = free_energy_simfold(partial_sequence, partial_structure);
  else
    energy = free_energy_simfold_restricted(partial_sequence, partial_structure,
                                            partial_restricted);
  if (removeAU) {
    if (has_AU_penalty(
            nuc_to_int(seq[i]),
            nuc_to_int(
                seq[j]))) // remove the AU penalty, because we don't know if
                          // this is going to be an exterior loop or not
    {
...
// return -1 if actual_num_str == MAXSUBSTR, meaning the value cannot be
// computed
{
  ...
  if (restricted == NULL) {
    ...
    // s_sub_folding* sub_fold = new s_sub_folding(sequence,
    // -(int)(min_energy*100.0));
    ...
    // THERE ARE SOME BUGS IN THE SUBOPTIMAL RESTRICTED, so I just compute the
    // unrestricted ones and eliminate what doesn't match
    ...
    actual_num_str2 =
        sub_fold->return_structures(tmp_structures2, tmp_energies2);
    delete sub_fold;

    // THE PROBLEM is this doesn't work either for the cases where hairpin loops
    // are shorter than 3
    /*
    s_sub_folding* sub_fold = new s_sub_folding(csequence, 10000);
    sub_fold->set_limit (MAXSUBSTR);
    sub_fold->s_simfold (enthalpy);
    actual_num_str2 = sub_fold->return_structures(tmp_structures2,
    tmp_energies2); delete sub_fold;
    */
  ...
  for (i = 0; i < actual_num_str; i++) {
    ...
    // if (!(i==54)) continue;
    // if (!(i==49 || i==50 || i==51 || i==53 || i==54 || i==55 || i==57))
    // continue; if (i==57 || i==52 || i==55 || i==54 || i==53 || i==51 ||
    // i==50) continue; if (i==56 || i==40 || i==39 || i==38 || i==31 || i==25
    // || i==15) continue; recompute the free energy, i.e. with the correct
    // dangling ends
    if (restricted == NULL)
      energy = free_energy_simfold(csequence, tmp_structures[i]);
    else
      energy = free_energy_simfold_restricted(csequence, tmp_structures[i],
                                              restricted);
  ...
  int l, identical;
  for (i = 0; i < seqlen; i++) {
    // Removed TURN on Apr 27, 2008. It shouldn't be there for the restricted
    // case;
    //       and I don't think it hurts not to be there for the unrestricted
    //       case.
    ...
    // for (j=i+TURN+1; j < seqlen; j++)
    {
      ...
      for (k = 0; k < actual_num_str; k++) {
        ...
        if (valid_structure(i, j, tmp_structures[k]) &&
            is_structured(i, j, tmp_structures[k])) {
          identical = 0;
          for (l = 0; l < k; l++) {
          ...
          if (!identical) {
            // printf ("Adding: %g\n", exp_free_energy_partial (i, j, csequence,
            // tmp_structures[k], 0));
            uexhaustive[ij] +=
        ...
        if (has_base_pair(i, j, tmp_structures[k])) {
          ...
          if (!identical) {
            upexhaustive[ij] +=
                exp_free_energy_partial(i, j, csequence, tmp_structures[k], 1);
          }
          // else
...
void s_partition_function::verify_simple_vs_complicated_bpp(
    s_partition_function *pf)
...
// function verifies that
{
  ...
  for (i = 0; i < seqlen; i++) {
    for (j = i + 1; j < seqlen; j++) {
      int ij = index[i] + j - i;
      if ((p[ij] - pf->p[ij] > PREC) || (pf->p[ij] - p[ij] > PREC)) {
        if (!error)
          printf("\n");
...
void s_partition_function::verify_simple_vs_complicated_recurrences(
    s_partition_function *pf)
// assume this one has ignore_dangles = 1 and otherpf has ignore_dangles=0
// assume that the input parameters for the object with ignore_dangles=0 has
...
// values this function verifies that
{
  ...
  for (i = 0; i < seqlen; i++) {
    for (j = i + 1; j < seqlen; j++) {
      int ij = index[i] + j - i;
      if ((u[ij] - (pf->u_ip_jp[ij] + pf->u_iu_jp[ij] + pf->u_ip_ju[ij] +
                    pf->u_iu_ju[ij]) >
           PREC) ||
          ((pf->u_ip_jp[ij] + pf->u_iu_jp[ij] + pf->u_ip_ju[ij] +
            pf->u_iu_ju[ij]) -
               u[ij] >
           PREC)) {
        if (!error)
    ...
    for (j = i + 1; j < seqlen; j++) {
      ...
      if ((s1[ij] - (pf->s1_jp[ij] + pf->s1_ju[ij]) > PREC) ||
        ...
        printf("DIFF! s1[%d,%d]=%g, s1_jx=%g, diff=%g\n", i, j, s1[ij],
               (pf->s1_jp[ij] + pf->s1_ju[ij]),
               (pf->s1_jp[ij] + pf->s1_ju[ij]) - s1[ij]);
        error = 1;
      }
    }
  }
  ...
  for (i = 0; i < seqlen; i++) {
    for (j = i + 1; j < seqlen; j++) {
      int ij = index[i] + j - i;
      if ((up[ij] - pf->up[ij] > PREC) || (pf->up[ij] - up[ij] > PREC)) {
        if (!error)
          printf("\n");
  ...
  for (i = 0; i < seqlen; i++) {
    for (j = i + 1; j < seqlen; j++) {
      int ij = index[i] + j - i;
      if ((upm[ij] - pf->upm[ij] > PREC) || (pf->upm[ij] - upm[ij] > PREC)) {
        if (!error)
          printf("\n");
        printf("DIFF! upm[%d,%d]=%g, upm=%g, diff=%g\n", i, j, upm[ij],
               pf->upm[ij], pf->upm[ij] - upm[ij]);
        error = 1;
      }
    }
  }
    ...
    for (j = i + 1; j < seqlen; j++) {
      ...
      if ((s2[ij] - (pf->s2_jp[ij] + pf->s2_ju[ij]) > PREC) ||
        ...
        printf("DIFF! s2[%d,%d]=%g, s2_jx=%g, diff=%g\n", i, j, s2[ij],
               (pf->s2_jp[ij] + pf->s2_ju[ij]),
               (pf->s2_jp[ij] + pf->s2_ju[ij]) - s2[ij]);
        error = 1;
      }
    }
  }
  ...
  for (i = 0; i < seqlen; i++) {
    for (j = i + 1; j < seqlen; j++) {
      int ij = index[i] + j - i;
      if ((u1[ij] - (pf->u1_ip_jp[ij] + pf->u1_iu_jp[ij] + pf->u1_ip_ju[ij] +
                     pf->u1_iu_ju[ij]) >
           PREC) ||
          ((pf->u1_ip_jp[ij] + pf->u1_iu_jp[ij] + pf->u1_ip_ju[ij] +
            pf->u1_iu_ju[ij]) -
               u1[ij] >
           PREC)) {
        if (!error)
    ...
    for (j = i + 1; j < seqlen; j++) {
      ...
      if ((s3[ij] - (pf->s3_jp[ij] + pf->s3_ju[ij]) > PREC) ||
        ...
        printf("DIFF! s3[%d,%d]=%g, s3_jx=%g, diff=%g\n", i, j, s3[ij],
               (pf->s3_jp[ij] + pf->s3_ju[ij]),
               (pf->s3_jp[ij] + pf->s3_ju[ij]) - s3[ij]);
        error = 1;
      }
    }
  }
...
// POST: returns 1 if there's error, 0 otherwise
{
  ...
  if (isnan(Z) || isnan(Zexhaustive))
    error = 1;
  // actually check the ratio, not the difference
  // the difference depends on the scale of Z , so then one Z is large, the
  // difference becomes large too, but it's good example: Z = 270.721, Ze =
  // 270.721, diff = -2.35534e-13, ratio = -8.70025e-16
...
void s_partition_function::verify_recurrences() {
  ...
  //         {
  //             ij = index[i]+j-i;
  //             if (u[ij] - uexhaustive[ij] > PREC || uexhaustive[ij] - u[ij] >
  //             PREC)
  //             {
  //                 if (ufine)
  //                     printf ("\t----------------------\n");
  //                 printf ("\tu[%d,%d] = %g, uex = %g, diff = %g\n",
  //                 i,j,u[ij],uexhaustive[ij], u[ij]-uexhaustive[ij]); ufine =
  //                 0;
  //             }
  //             //printf ("\tu[%d,%d] = %g, uex = %g, diff = %g\n",
    ...
    for (j = i + 1; j < seqlen; j++) {
      ...
      if (isnan(up[ij]) || isnan(upexhaustive[ij])) {
        printf("\tup[%d,%d] = %g, upex = %g, diff = %g\n", i, j, up[ij],
               upexhaustive[ij], up[ij] - upexhaustive[ij]);
        upfine = 0;
        error = 1;
      }
      if ((up[ij] - upexhaustive[ij]) / up[ij] > PREC ||
          (upexhaustive[ij] - up[ij]) / up[ij] > PREC) {
        if (upfine)
          printf("\t----------------------\n");
        printf("\tup[%d,%d] = %g, upex = %g, diff = %g, ratio = %g\n", i, j,
               up[ij], upexhaustive[ij], up[ij] - upexhaustive[ij],
               (up[ij] - upexhaustive[ij]) / up[ij]);
        upfine = 0;
        error = 1;
      }
      // printf ("\tup[%d,%d] = %g, upex = %g, diff = %g\n",
  ...
  if (upfine)
    printf("\t++++++++++++++++++++++\n");
  // NOTE: this will be different when we have the special GGG-U hairpin loop.
  // It's okay to be different, it doesn't affect the partition function.
    ...
    for (j = i + 1; j < seqlen; j++) {
      ...
      if (isnan(p[ij]) || isnan(pexhaustive[ij])) {
        printf("\tp[%d,%d] = %g, pex = %g, diff = %g\n", i, j, p[ij],
               pexhaustive[ij], p[ij] - pexhaustive[ij]);
        pfine = 0;
        error = 1;
      }
  ...
  for (i = 0; i < seqlen; i++)
    for (j = i + 1; j < seqlen; j++) {
      ...
      PFTYPE unew = 0;
      IFD unew = u[ij];
      else unew = u_ip_jp[ij] + u_ip_ju[ij] + u_iu_jp[ij] + u_iu_ju[ij];
      if (isnan(unew) || isnan(uexhaustive[ij])) {
        printf("\tunew[%d,%d] = %g, u = %g, diff = %g\n", i, j, unew,
               uexhaustive[ij], unew - uexhaustive[ij]);
        ufine = 0;
        error = 1;
      }
      if ((uexhaustive[ij] - unew) / unew > PREC ||
          (unew - uexhaustive[ij]) / unew > PREC) {
        if (ufine)
          printf("\t----------------------\n");
        printf("\tunew[%d,%d] = %g, u = %g, diff = %g, ratio = %g\n", i, j,
               unew, uexhaustive[ij], unew - uexhaustive[ij],
               (unew - uexhaustive[ij]) / unew);
  ...
  i,j,s1new,s1[ij], s1new-s1[ij]); s1fine = 0;
          }
      }
  if (s1fine)
      printf ("\t++++++++++++++++++++++\n");
  */
  ...
  //         {
  //             ij = index[i]+j-i;
  //             //u1 should be the same as u1_ip_jp + u1_ip_ju + u1_iu_jp +
  //             u1_iu_ju double u1new = u1_ip_jp[ij] + u1_ip_ju[ij] +
  //             u1_iu_jp[ij] + u1_iu_ju[ij]; if (u1[ij] - u1new > PREC || u1new
  //             - u1[ij] > PREC)
  //             {
  //                 if (u1fine)
  //                     printf ("\t----------------------\n");
  //                 printf ("\tu1new[%d,%d] = %g, u1 = %g, diff = %g\n",
  //                 i,j,u1new,u1[ij], u1new-u1[ij]); printf ("\t  u1_ip_jp=%g,
  //                 u1_ip_ju=%g, u1_iu_jp=%g, u1_iu_ju=%g\n", u1_ip_jp[ij],
  //                 u1_ip_ju[ij], u1_iu_jp[ij], u1_iu_ju[ij]); u1fine = 0;
  //             }
  //         }
  //     if (u1fine)
  //         printf ("\t++++++++++++++++++++++\n");
  ...
  //                 i,j,s3new,s3[ij], s3new-s3[ij]); s3fine = 0;
  //             }
  //         }
  //     if (s3fine)
  //         printf ("\t++++++++++++++++++++++\n");
...
void s_partition_function::compute_logZ_gradient_exhaustively() {
  ...
  if (restricted == NULL) {
    ...
    double tmp_energies2[MAXSUBSTR];
    int actual_num_str2;
    // THERE ARE SOME BUGS IN THE SUBOPTIMAL RESTRICTED, so I have to eliminate
    // what doesn't match
  ...
  for (k = 0; k < actual_num_str; k++) {
    ...
    if (restricted == NULL) {
      ...
      count_each_structure_type(csequence, tmp_structures[k], "", counter, f,
                                1);
    } else {
      energy = free_energy_simfold_restricted(csequence, tmp_structures[k],
                                              restricted);
      denominator += EXP((-1) * energy * beta);
      count_each_structure_type(csequence, tmp_structures[k], restricted,
                                counter, f, 1);
    }
...
// u, up and p arrays are filled
{
        ...
        for (l = 0; l < NUCL; l++) {
          if (stack[i][j][k][l] < INF) {
            ...
            if (i * 1000 + j * 100 + k * 10 + l <=
                  ...
                  if (sequence[ii] == i && sequence[jj] == j) {
                    ...
                    for (iip = ii + 1; iip <= ii + 2; iip++) {
                      for (jjp = jj - 1; jjp >= jj - 2; jjp--) {
                        if (iip == ii + 2 &&
                            jjp == jj - 2) // this is not stacking energy, is
                                           // internal loop 1x1
                          continue;
                        if (sequence[iip] == k && sequence[jjp] == l) {
                          ...
                          if (iip == ii + 1 && jjp == jj - 1) // stack pair
                            en_stack =
                                s_stacked_pair::get_energy(ii, jj, sequence);
                          else // bulge
                          {
                            if (ignore_internal)
                              continue;
                            en_stack = s_internal_loop::get_energy(
                                ii, jj, iip, jjp, sequence, ptable_restricted);
                          }
                          GlogZ[index_param] += p[iijj] * up[iipjjp] /
                                                up[iijj] *
                                                EXP(en_stack * oneoverRT);
                        }
                      }
                    }
                  }
              ...
              // check if it's symmetric. If so, it was already considered when
              // we looked from the left
                ...
                // for (ii = seqlen-1; ii > TURN; ii--)
                {
                  ...
                  // for (jj = ii-TURN-1; jj > 0; jj--)
                  {
                    if (sequence[ii] == i && sequence[jj] == j) {
                      ...
                      // stacking energies are also involved in bulges of size 1
                      for (iip = ii + 1; iip <= MIN(ii + 2, seqlen - 1);
                           iip++) {
                        for (jjp = jj - 1; jjp >= MAX(jj - 2, 0); jjp--) {
                          ...
                          if (sequence[iip] == k && sequence[jjp] == l) {
                            ...
                            if (up[iipjjp] == 0)
                              continue;
                            if (iip == ii + 1 && jjp == jj - 1) // stack pair
                              en_stack = s_stacked_pair::get_energy(jjp, iip,
                                                                    sequence);
                            else // bulge
                            {
                              if (ignore_internal)
                                continue;
                              en_stack = s_internal_loop::get_energy(
                                  jjp, iip, jj, ii, sequence,
                                  ptable_restricted);
                            }
                            GlogZ[index_param] += p[iipjjp] * up[iijj] /
                                                  up[iipjjp] *
                                                  EXP(en_stack * oneoverRT);
                          }
                        }
            ...
            for (ii = 0; ii < seqlen; ii++) {
              ...
              // for (jj = ii+TURN+1; jj < seqlen; jj++)
              {
                if (sequence[ii] == i && sequence[jj] == j &&
                  ...
                  if (up[iijj] == 0)
                    continue;
                  en_hairpin = s_hairpin_loop::get_energy(
                      ii, jj, sequence, csequence, ptable_restricted);
                  PFTYPE grad =
                      p[iijj] / up[iijj] * EXP(en_hairpin * oneoverRT);
                  // tried to get a better precision, but it doesn't seem to
                  // help
                  // double grad = p[iijj] / up[iijj] * pow(exp (en_hairpin *
                  // oneoverRT/10.0), 10.0);
                  ...
                  // In s_hairpin_loop.cpp, we decided not to add AU_penalty in
                  // this case
                  if (restricted != NULL && jj - ii - 1 < 3) {
                    //                                         if
                    //                                         (ptable_restricted[ii]
                    //                                         == jj)
                    //                                         {
                    //                                             if
                    //                                             (has_AU_penalty
                  ...
                  // I changed TURN to 0 for the restricted cases, so I have to
                  // make sure I don't look for hairpin_penalty_by_size[0] or
                  // [1] or [2]
                  if (jj - ii - 1 >= MIN_HAIRPIN) {
                    sprintf(s, "hairpin_penalty_by_size[%d]", jj - ii - 1);
                    // this is very slow, it's linear in the number of
                    // parameters
                  ...
                  if (ii > 1) {
                    if (sequence[ii] == G && sequence[ii - 1] == G &&
                        sequence[ii - 2] == G and sequence[jj] == U) {
                  ...
                  if (is_poly_C) {
                    if (jj - ii - 1 == 3) {
                      ...
                      // we have to multiply by size because this param is
                      // multiplied by size in the hairpin loop calculation
            ...
            // if (tstacki[i][j][k][l] < INF)
            {
              ...
              for (ii = 0; ii < seqlen; ii++) {
                ...
                // for (jj = ii+TURN+4; jj < seqlen; jj++)
                {
                  ...
                  if (sequence[ii] == i && sequence[jj] == j && k == 0 &&
                      l == 0) {
                    iijj = index[ii] + jj - ii;
                    if (up[iijj] == 0)
                      continue;
                    // Removed TURN on Apr 27, 2008
                    for (iip = ii + 1; iip <= MIN(jj - 2, ii + MAXLOOP + 1);
                         iip++) // j-2-TURN
                    // for (iip = ii+1; iip <= MIN(jj-2-TURN,ii+MAXLOOP+1) ;
                    // iip++)  // j-2-TURN
                    {
                      // Removed TURN on Apr 27, 2008
                      int minq = MAX(jj - ii + iip - MAXLOOP - 2,
                                     iip + 1); // ip+1+TURN);
                      // int minq = MAX (jj-ii+iip-MAXLOOP-2, iip+1+TURN);    //
                      // ip+1+TURN);
                      for (jjp = minq; jjp < jj; jjp++) {
                        if (sequence[iip] + sequence[jjp] == 3 ||
                          ...
                          if (((branch1 == 1 && branch2 > 2) ||
                              ...
                              misc.gail_rule) {
                            // char s[100];
                            sprintf(s, "internal_penalty_by_size[%d]",
                                    branch1 + branch2);
                            // this is very slow, it's linear in the number of
                            // parameters
                            int sizeindex = structure_type_index(s);
                            ...
                            en_internal = s_internal_loop::get_energy(
                                ii, jj, iip, jjp, sequence, ptable_restricted);
                            PFTYPE grad = p[iijj] * up[iipjjp] / up[iijj] *
                                          EXP(en_internal * oneoverRT);
                            GlogZ[sizeindex] += grad;

                            // now check if any of the 3 internal params is
                            // involved
                            if (((i == A || i == G) && j == U) ||
                                ((j == A || j == G) && i == U)) {
                              // GlogZ[index_terminal_AU_penalty] += p[iijj] *
                              // up[iipjjp] / up[iijj] * exp (en_internal *
                              // oneoverRT);
                              GlogZ[index_internal_AU_closure] += grad;
                            }
                            // AG and UU mismatch can't happen here, because k
                            // is A and l is A
                          }
                        }
                      }
                      ...
                      for (jjp = minq; jjp < jj; jjp++) {
                        if (sequence[iip] + sequence[jjp] == 3 ||
                          ...
                          if (branch1 < 0 || branch2 < 0) // not valid
                            continue;
                          if (branch1 == 0 &&
                              branch2 == 0) // stack pair, not good here
                            continue;
                          if (branch1 == 1 && branch2 == 1 &&
                              !simple_internal_energy)
                            continue;
                          if (branch1 == 1 && branch2 == 2 &&
                              !simple_internal_energy)
                            continue;
                          if (branch1 == 2 && branch2 == 1 &&
                              !simple_internal_energy)
                            continue;
                          ...
                          if (branch1 == 0 || branch2 == 0) // bulge
                          {
                            ...
                            // char s[100];
                            sprintf(s, "bulge_penalty_by_size[%d]",
                                    branch1 + branch2);
                            // this is very slow, it's linear in the number of
                            // parameters
                            int sizeindex = structure_type_index(s);
                            ...
                            if (branch1 + branch2 > 1) // add AU_penalty
                            {
                              ...
                              if (has_AU_penalty(sequence[iip],
                                                 sequence[jjp])) {
                                GlogZ[AUpen_index] += grad;
                              }
                            }
                          ...
                          // Apr 28, 2008: I don't know why this was here, I
                          // think it should be later below
              ...
              // for (ii = seqlen-2; ii > TURN; ii--)
              {
                ...
                // for (jj = ii-TURN-1; jj > 0; jj--)
                {
                  ...
                  if (sequence[ii] == i && sequence[jj] == j && k == 0 &&
                    ...
                    for (iip = ii + 1; iip <= MIN(seqlen - 1, ii + MAXLOOP + 1);
                         iip++) // j-2-TURN
                    {
                      int minq =
                          MAX(jj - ii + iip - MAXLOOP - 2, 0); // ip+1+TURN);
                      for (jjp = minq; jjp < jj; jjp++) {
                        if (sequence[iip] + sequence[jjp] == 3 ||
                            sequence[iip] + sequence[jjp] == 5) {
                          int branch1 = iip - ii - 1;
                          ...
                          if (((branch1 == 1 && branch2 > 2) ||
                              ...
                              misc.gail_rule) {
                            en_internal = s_internal_loop::get_energy(
                                jjp, iip, jj, ii, sequence, ptable_restricted);
                            // now check if any of the 3 internal params is
                            // involved
                            if (((i == A || i == G) && j == U) ||
                                ((j == A || j == G) && i == U)) {
                              // GlogZ[index_terminal_AU_penalty] += p[iipjjp] *
                              // up[iijj] / up[iipjjp] * exp (en_internal *
                              // oneoverRT);
                              GlogZ[index_internal_AU_closure] +=
                                  p[iipjjp] * up[iijj] / up[iipjjp] *
                                  EXP(en_internal * oneoverRT);
                            }
                            // AG and UU mismatch can't happen here, because k
                            // is A and l is A
                         ...
                         iip++) // j-2-TURN
                    {
                      ...
                      for (jjp = minq; jjp < jj; jjp++) {
                        if (sequence[iip] + sequence[jjp] == 3 ||
                          ...
                          en_internal = s_internal_loop::get_energy(
                              jjp, iip, jj, ii, sequence, ptable_restricted);
                          PFTYPE grad = p[iipjjp] * up[iijj] / up[iipjjp] *
                                        EXP(en_internal * oneoverRT);
                ...
                for (n = 0; n < NUCL; n++) {
                  if (int11[i][j][k][l][m][n] < INF) {
                    ...
                    if (i * 100000 + j * 10000 + k * 1000 + l * 100 + m * 10 +
                      ...
                      {
                        if (sequence[ii] == i && sequence[ii + 1] == k &&
                            sequence[ii + 2] == m) {
                          // Removed TURN on Apr 27, 2008
                          for (jj = ii + 5; jj < seqlen; jj++)
                          // for (jj = ii + TURN+5; jj < seqlen; jj++)
                          {
                            if (sequence[jj] == j && sequence[jj - 1] == l &&
                                sequence[jj - 2] == n) {
                              iijj = index[ii] + jj - ii;
                              if (up[iijj] == 0)
                                continue;
                              ...
                              if (((iinuc == C && jjnuc == G) ||
                                ...
                                if (!can_pair(kknuc, llnuc))
                                  sprintf(type, "int11[%d][%d][%d][%d][%d][%d]",
                                          iinuc, jjnuc, kknuc, llnuc, mmnuc,
                                          nnnuc);
                                else
                                  sprintf(type,
                                ...
                                GlogZ[tindex] += grad;
                              } else if (watson_crick(iinuc, jjnuc) &&
                                         watson_crick(mmnuc, nnnuc) &&
                                         kknuc == U && llnuc == U) {
                                sprintf(type, "int11[%d][%d][%d][%d][%d][%d]",
                                        iinuc, jjnuc, kknuc, llnuc, mmnuc,
                                        nnnuc);
                            ...
                            // for (jj = ii-TURN-1; jj >= 2; jj--)
                            {
                              if (sequence[jj] == j && sequence[jj - 1] == l &&
                                ...
                                if (up[iipjjp] == 0)
                                  continue;
                                en_internal = s_internal_loop::get_energy(
                                    jj - 2, ii + 2, jj, ii, sequence,
                                    ptable_restricted);
                                PFTYPE grad = p[iipjjp] * up[iijj] /
                                              up[iipjjp] *
                                              EXP(en_internal * oneoverRT);
                        ...
                        if (sequence[ii] == i && sequence[ii + 1] == k &&
                          ...
                          // for (jj = ii + TURN+6; jj < seqlen; jj++)
                          {
                            if (sequence[jj] == j && sequence[jj - 1] == l &&
                              ...
                              if ((iinuc == C && jjnuc == G && mmnuc == C &&
                                   nnnuc ==
                                       G) || // these are already filled above,
                                             // except what can pair inside
                                  (iinuc == G && jjnuc == C && mmnuc == G &&
                                   nnnuc == C)) {
                                if (can_pair(kknuc, llnuc) ||
                                    can_pair(kknuc, oonuc))
                                ...
                                if (can_pair(kknuc, llnuc) ||
                                  ...
                                  sprintf(type,
                                          "int21[%d][%d][%d][%d][%d][%d][%d]",
                                          C, G, kknuc, llnuc, C, G, oonuc);
                                  tindex = structure_type_index(type);
                                  GlogZ[tindex] += 0.5 * grad;

                                  sprintf(type,
                                          "int21[%d][%d][%d][%d][%d][%d][%d]",
                                          G, C, kknuc, llnuc, G, C, oonuc);
                                  tindex = structure_type_index(type);
                                  GlogZ[tindex] += 0.5 * grad;
                                }
                                if (has_AU_penalty(iinuc, jjnuc)) {
                    ...
                    for (r = 0; r < NUCL; r++) {
                      if (int22[i][j][k][l][m][n][o][r] < INF) {
                        ...
                        if (i * 10000000 + j * 1000000 + k * 100000 +
                                l * 10000 + m * 1000 + n * 100 + o * 10 + r <=
                            n * 10000000 + m * 1000000 + r * 100000 +
                            ...
                            if (sequence[ii] == i && sequence[ii + 1] == k &&
                              ...
                              // for (jj = ii + TURN+7; jj < seqlen; jj++)
                              {
                                if (sequence[jj] == j &&
                                  ...
                                  en_internal = s_internal_loop::get_energy(
                                      ii, jj, ii + 3, jj - 3, sequence,
                                      ptable_restricted);
                                  PFTYPE grad = p[iijj] * up[iipjjp] /
                                                up[iijj] *
                                                EXP(en_internal * oneoverRT);
                                  ...
                                  if (nnnuc == iinuc && mmnuc == jjnuc &&
                                      ppnuc == kknuc &&
                                      oonuc == llnuc &
                                          watson_crick(iinuc, jjnuc) &&
                                      !watson_crick(kknuc, llnuc)) {
                                    sprintf(
                                        type,
                                  ...
                                  if (watson_crick(kknuc, llnuc) ||
                                      watson_crick(oonuc, ppnuc)) {
                                    sprintf(type, "misc.internal22_match");
                                    tindex = structure_type_index(type);
                                    GlogZ[tindex] += grad;
                                  } else if (((iinuc == G && jjnuc == U) ||
                                              (iinuc == U && jjnuc == G) ||
                                              (mmnuc == G && nnnuc == U) ||
                                              (mmnuc == U && nnnuc == G)) &&
                                             (nnnuc2 == iinuc2 &&
                                                               ...
                                                               // same as UA
                                  {
                                    sprintf(
                                        type,
                                        "int22[%d][%d][%d][%d][%d][%d][%d][%d]",
                                        iinuc2, jjnuc2, kknuc, llnuc, mmnuc2,
                                        nnnuc2, oonuc, ppnuc);
                                    tindex = structure_type_index(type);
                                    GlogZ[tindex] += grad;
                                  } else if (!(nnnuc == iinuc &&
                                               mmnuc == jjnuc &&
                                               ppnuc == kknuc &&
                                               oonuc == llnuc)) // was already
                                                                // filled above
                                  {
                                    sprintf(
                                        type,
                                        "int22[%d][%d][%d][%d][%d][%d][%d][%d]",
                                        iinuc2, jjnuc2, kknuc, llnuc, jjnuc2,
                                        iinuc2, llnuc, kknuc);
                                    tindex = structure_type_index(type);
                                    GlogZ[tindex] += 0.5 * grad;
                                    sprintf(
                                        type,
                                        "int22[%d][%d][%d][%d][%d][%d][%d][%d]",
                                        nnnuc2, mmnuc2, ppnuc, oonuc, mmnuc2,
                                        nnnuc2, oonuc, ppnuc);
                                    tindex = structure_type_index(type);
                                    GlogZ[tindex] += 0.5 * grad;

                                    int result = check_stability_and_size(
                                        kknuc, llnuc, oonuc, ppnuc);
                                    switch (result) {
                                    case 1:
                                      sprintf(
                                          type,
                                      ...
                                      break;
                                    case 2:
                                      sprintf(type, "misc.internal22_delta_"
                                                    "different_size");
                                      break;
                                    case 3:
                                      sprintf(type, "misc.internal22_delta_"
                                    ...
                                    case 4:
                                      ...
                                      break;
                                    default:
                                      printf("ERROR: result %d for k=%d, l=%d, "
                                             "o=%d, p=%d, ABORT!\n",
                                             result, kknuc, llnuc, oonuc,
                                             ppnuc);
                                      exit(1);
                                    }
                                    tindex = structure_type_index(type);
                                ...
                                // for (jj = ii-TURN-1; jj >= 3; jj--)
                                {
                                  if (sequence[jj] == j &&
                                    ...
                                    if (up[iipjjp] == 0)
                                      continue;
                                    en_internal = s_internal_loop::get_energy(
                                        jj - 3, ii + 3, jj, ii, sequence,
                                        ptable_restricted);
                                    PFTYPE grad = p[iipjjp] * up[iijj] /
                                                  up[iipjjp] *
                                                  EXP(en_internal * oneoverRT);

                                    int iinuc, jjnuc, kknuc, llnuc, mmnuc,
                                        nnnuc, oonuc, ppnuc;
                                    iinuc = sequence[ii];
                                    jjnuc = sequence[jj];
                                    kknuc = sequence[ii + 1];
                                                            ...
                                                            // same as UA
                                    {
                                      sprintf(type,
                                              "int22[%d][%d][%d][%d][%d][%d][%"
                                              "d][%d]",
                                              iinuc2, jjnuc2, kknuc, llnuc,
                                              mmnuc2, nnnuc2, oonuc, ppnuc);
                                      tindex = structure_type_index(type);
                                      GlogZ[tindex] += grad;
                                    } else if (!(nnnuc == iinuc &&
                                                 mmnuc == jjnuc &&
                                                             ...
                                                             // filled above
                                    {
                                      sprintf(type,
                                              ...
                                              "d][%d]",
                                              iinuc2, jjnuc2, kknuc, llnuc,
                                              jjnuc2, iinuc2, llnuc, kknuc);
                                      tindex = structure_type_index(type);
                                      GlogZ[tindex] += 0.5 * grad;
                                      sprintf(type,
                                              "int22[%d][%d][%d][%d][%d][%d][%"
                                              "d][%d]",
                                              nnnuc2, mmnuc2, ppnuc, oonuc,
                                              mmnuc2, nnnuc2, oonuc, ppnuc);
                                      tindex = structure_type_index(type);
                                      GlogZ[tindex] += 0.5 * grad;
      ...
      for (j = 0; j < NUCL; j++)
        for (k = 0; k < NUCL; k++) {
          if (dangle_top[i][j][k] < INF) {
            ...
            // first add the dangling energies that participate to the external
            // loop
            for (ii = 0; ii < seqlen; ii++) {
              for (jj = ii + TURN + 1; jj < seqlen - 1; jj++) {
                if (sequence[jj] == i && sequence[ii] == j &&
                    sequence[jj + 1] == k) {
                  PFTYPE term1 = 0.0;
                  PFTYPE term2 = 0.0;
                  ...
                  if (ii > TURN)
                    term1 += u_ip_jp[ii - 1] + u_iu_jp[ii - 1] +
                             exp_dangle5(jj, ii, ii - 1) *
                                 (u_ip_ju[ii - 1] + u_iu_ju[ii - 1]);
                  else if (ii > 0)
                    term1 += exp_dangle5(jj, ii, ii - 1);
                  else
                    term1 = 1;

                  if (jj < seqlen - 3) {
                    int jjp2n = index[jj + 2] + seqlen - 1 - (jj + 2);
                    term2 += exp_dangle3(jj, ii, jj + 1) *
                             (u_ip_jp[jjp2n] + u_ip_ju[jjp2n] + u_iu_jp[jjp2n] +
                              u_iu_ju[jjp2n]);
                  } else if (jj < seqlen - 1)
                    term2 += exp_dangle3(jj, ii, jj + 1);
                  ...
                  GlogZ[index_param] +=
                      term1 * up[iijj] * term2 * exp_AUpenalty(ii, jj);
                }
              }
            }
            // moved the denominator outside, it's the same everywhere
            GlogZ[index_param] /= (u_ip_jp[seqlen - 1] + u_ip_ju[seqlen - 1] +
                                   u_iu_jp[seqlen - 1] + u_iu_ju[seqlen - 1]);
              ...
              for (ii = jj - TURN - 1; ii >= 0; ii--) {
                ...
                if (sequence[ii] == i && sequence[jj] == j &&
                       ...
                       l++) // case (.(...)--(--)-)
                  {
                    ...
                    temp +=
                        up[iip2l] * exp_AUpenalty(ii + 2, l) *
                        (u1_ip_jp[lp1jjm1] // [.(...)(.-..)] or [.(...)(.-..).]
                         + exp_dangle3(l, ii + 2, l + 1) * EXPC[1] *
                               (u1_ip_jp[lp2jjm1] +
                                u1_iu_jp[lp2jjm1] + // [.(...).-(...)] or
                                                    // [.(...).-(...).]
                                exp_dangle5(ii, jj, jj - 1) *
                                    (u1_ip_ju[lp2jjm1] +
                                     u1_iu_ju[lp2jjm1])) + // [.(...).-(...)-..]
                         +exp_dangle5(ii, jj, jj - 1) *
                             u1_ip_ju[lp1jjm1]); // [.(...)(...)-..]
                  }
                  for (h = ii + 3; h < jj - TURN - 2;
                       h++) // case (....(...)--(--)-)
                  {
                  ...
                  GlogZ[index_param] +=
                      temp * EXPA * EXPB2 * EXPC[1] * exp_AUpenalty(ii, jj) *
                      exp_dangle3(ii, jj, ii + 1) * p[iijj] / up[iijj];
                }
              }
            }

            // the multi-loop 3' dangling end which can appear to the right of
            // branches taken from compute_p
            temp = 0;
            for (h = 0; h < seqlen; h++) {
              for (l = seqlen - 2; l > h + TURN;
                   l--) // it's seqlen-2 because l+1 must be valid
              {
                // printf ("h=%d, l=%d, seqlen=%d\n", h, l, seqlen);
                if (can_pair(sequence[h], sequence[l]) && sequence[l] == i &&
                    sequence[h] == j && sequence[l + 1] == k) {
                  PFTYPE temp = 0;
                  int hl = index[h] + l - h;
                  ...
                  for (ii = 0; ii < h; ii++) {
                    ...
                    // the case when h-l is the first branch of the multi-loop
                    if (ii < h - 1) // we must add d3 (which is added in pmd3);
                                    // also add d5 here, if i < h-2
                    {
                      // removed one line, no dangling end of interest
                      temp += EXPC[h - ii] *
                              pmd3_needmidd3[iil] * // EXPC[1] * // added it to
                                                    // the left
                    ...
                    {
                      // removed one line, no dangling end of interest
                      temp += pmnod3_needmidd3[iil] *
                              EXPC[h - ii]; // * EXPC[1];
                                            // ([...].-(---)-) ih...l.-(---)-j
                    }

                    if (ii < h - TURN -
                                 2) // only now we can have a branch to the left
                    {
                      ...
                      temp +=
                          ...
                          // we have branches to the left and to the right of
                          // h-l if this is changed in compute_p, it should be
                          // changed here too
                          //+ pmnod3_noneedmidd3[iil]  * u1[iip1hm1]
                          // ((...)[...]-(---)...)    i(...)h...l-(---)...j
                          //+ pmnod3_needmidd3[iil] * exp_dangle3(l,h,l+1) *
                          // u1[iip1hm1]);
                          // ((...).-[...]-(---)...)    i(...).-h...l-(---)...j

                          + pmnod3_needmidd3[iil] *
                                EXPC[1] * //+ pmnod3_noneedmidd3[il]) *
                                (u1_ip_jp[iip1hm1] +
                                 exp_dangle5(l, h, h - 1) * u1_ip_ju[iip1hm1])
                          // c is added in pm1nod3_needendd3
                          // ((...).-[...].-(--)-)         i(...)h...l.-j
                  ...
                  GlogZ[index_param] += up[hl] * EXPA * EXPB2 *
                                        exp_AUpenalty(h, l) *
      ...
      for (j = 0; j < NUCL; j++)
        for (k = 0; k < NUCL; k++) {
          if (dangle_bot[i][j][k] < INF) {
            ...
            // no duplicates here
            // first add the dangling energies that participate to the external
            // loop traverse the whole sequence and look for this building block
                ...
                if (sequence[jj] == i && sequence[ii] == j &&
                  ...
                  if (jj < seqlen - 3) {
                    ...
                    term2 += (u_ip_jp[jjp1n] + u_ip_ju[jjp1n] +
                              exp_dangle3(jj, ii, jj + 1) *
                                  (u_ip_jp[jjp2n] + u_ip_ju[jjp2n] +
                                   u_iu_jp[jjp2n] + u_iu_ju[jjp2n]));
                  } else if (jj < seqlen - 1)
                    term2 += exp_dangle3(jj, ii, jj + 1);
                  else
                    term2 = 1;
            ...
            for (jj = TURN + 1; jj < seqlen; jj++) {
              for (ii = jj - TURN - 1; ii >= 0; ii--) {
                ...
                if (sequence[ii] == i && sequence[jj] == j &&
                    sequence[jj - 1] == k) {
                  temp = 0;
                  // there must be at least one more branch (i.e. 5 nucleotides)
                  // from l+1 to j-1, plus at least 2 free bases: l+1+TURN <=
                  // j-3 removed the cases where there isn't a 5' dangling end
                  // on the last free base
                  for (l = ii + 2; l < jj - TURN - 4;
                       l++) // case ((...)--(--)-..)
                  {
                    ...
                    int lp2jjm1 = index[l + 2] + jj - 1 - l - 2;
                    temp +=
                        up[iip1l] * exp_AUpenalty(ii + 1, l) *
                        ( // u1_ip_jp[lp1jjm1] +     // no dangling end of
                          // interest [(...)(.-..)] or [(...)(.-..).]
                            exp_dangle3(l, ii + 1, l + 1) * EXPC[1] *
                                ( // u1_ip_jp[lp2jjm1] + u1_iu_jp[lp2jjm1] + //
                                  // [(...).-(...)] or [(...).-(...).]
                                    (u1_ip_ju[lp2jjm1] +
                                     u1_iu_ju[lp2jjm1])) + // [(...).-(...)-..]
                            +u1_ip_ju[lp1jjm1]);           // [(...)(...)-..]
                  ...
                  GlogZ[index_param] +=
                      temp * EXPA * EXPB2 * exp_dangle5(ii, jj, jj - 1) *
                      exp_AUpenalty(ii, jj) * p[iijj] / up[iijj];
                }
              }
            }

            // the multi-loop 5' dangling end which can appear to the left of
            // branches taken from compute_p
            temp = 0;

            for (h = 1; h < seqlen;
                 h++) // starts from 1 because h-1 must be valid
            {
              for (l = seqlen - 1; l > h + TURN; l--) {
                if (can_pair(sequence[h], sequence[l]) && sequence[l] == i &&
                    sequence[h] == j && sequence[h - 1] == k) {
                  ...
                  for (ii = 0; ii < h; ii++) {
                    int iil = index[ii] + l - ii;
                    // the case when h-l is the first branch of the multi-loop
                    if (ii < h - 2) // we must add d3 (which is added in pmd3);
                                    // also add d5 here, if i < h-2
                    {
                      temp += EXPC[h - ii - 1] *
                              (pmd3_noneedmidd3[iil] +
                               pmd3_needmidd3[iil] * exp_dangle3(l, h, l + 1) *
                                   EXPC[1]);
                      // (.-[...](---)-)    i.-h...l(---)-j
                      // (.-[...].-(---)-)    i.-h...l.-(---)-j
                    }
                                 ...
                                 2) // only now we can have a branch to the left
                    {
                      ...
                      if (up[iilp1] != 0) {
                        // printf ("EXPC=%g\n", exp
                        // ((misc.multi_free_base_penalty) * oneoverRT) );
                        term1 =
                            p[iilp1] / up[iilp1] * exp_AUpenalty(ii, l + 1) *
                      ...
                      temp +=
                          ...
                          (term1 +
                           ...
                           // case ((..-)-[...].-)
                           pm1nod3_needendd3[iil] *
                               exp_dangle3(l, h,
                                           l + 1) * // EXPC added in pm1nod3
                               u1_ip_ju[iip1hm1]
                           // c is added in pm1nod3_needendd3
                           // ((...).-[...].-)         i(...)h...l.-j
                           // case (.(..-)-[...].-)
  ...
  // the contribution from hairpin loop of size 3 (and below, for the restricted
  // case) was computed above now consider the contributions from the exterior
  // loop
  ...
  for (ii = 0; ii < seqlen; ii++) {
    ...
    // for (jj = ii+TURN+1; jj < seqlen; jj++)
    {
      if (can_pair(sequence[ii], sequence[jj]) &&
          has_AU_penalty(sequence[ii], sequence[jj])) {
        iijj = index[ii] + jj - ii;
        ...
        IFD {
          if (jj < seqlen - 1)
            term2 = u[index[jj + 1] + seqlen - 1 - (jj + 1)];
          else
            term2 = 1.0;
        }
        else {
          ...
          if (jj < seqlen - 3)
            term2 +=
                u_ip_jp[jjp1n] + u_ip_ju[jjp1n] +
                exp_dangle3(jj, ii, jj + 1) * (u_ip_jp[jjp2n] + u_ip_ju[jjp2n] +
                                               u_iu_jp[jjp2n] + u_iu_ju[jjp2n]);
          else if (jj < seqlen - 1)
            term2 += exp_dangle3(jj, ii, jj + 1);
          else
        ...
        IFD {
          if (u[seqlen - 1] != 0)
            GlogZ[AUpen_index] += term1 * up[iijj] * term2 *
                                  exp_AUpenalty(ii, jj) / u[seqlen - 1];
        }
        else {
          GlogZ[AUpen_index] += term1 * up[iijj] * term2 *
                                exp_AUpenalty(ii, jj) /
                                (u_ip_jp[seqlen - 1] + u_ip_ju[seqlen - 1] +
                                 u_iu_jp[seqlen - 1] + u_iu_ju[seqlen - 1]);
        }
        // if it closes a multi loop, or closes a helix of a multi-loop - it's
        // considered at helix below
      }
    }
  }
  ...
  if (!ignore_multi) {
    ...
    for (h = 0; h < seqlen; h++) {
      ...
      // for (l = h+TURN+1; l < seqlen; l++)
      {
        ...
        if (has_AU_penalty(sequence[h], sequence[l]))
          GlogZ[AUpen_index] += temp;

        temp = 0;
        for (i = 0; i < h; i++) {
          int il = index[i] + l - i;
          IFD { temp += EXPC[h - i - 1] * pm[il]; }
          else {
            if (i < h - 1) // we must add d3 (which is added in pmd3); also add
                           // d5 here, if i < h-2
            {
              temp += EXPC[h - i - 1] *
          ...
          // Removed TURN on Apr 27, 2008
          if (i < h - 2) // only now we can have a branch to the left
          // if (i < h - TURN -2)    // only now we can have a branch to the
          // left
          {
            int ip1hm1 = index[i + 1] + h - 1 - (i + 1);

            IFD { temp += u1[ip1hm1] * (pm1[il] + pm[il]); }
            else {
              ...
              PFTYPE term1 = 0.0;
              if (up[ilp1] != 0) {
                // printf ("EXPC=%g\n", exp ((misc.multi_free_base_penalty) *
                // oneoverRT) );
                term1 = p[ilp1] / up[ilp1] * exp_AUpenalty(i, l + 1) *
                        (                     // first, the case ((..-)-[...])
                            (u1_ip_jp[ip1hm1] // ((..-)[...]) or ((..-).[...])
              ...
              temp +=
                  ...
                  (term1 +
                   ...
                   pm1nod3_needendd3[il] *
                       exp_dangle3(l, h,
                                   l + 1) * // we don't need EXPC[1] here,
                                            // because it's in pm1nod3_needendd3
                       (u1_ip_jp[ip1hm1] +
                        exp_dangle5(l, h, h - 1) * u1_ip_ju[ip1hm1])
                   // c is added in pm1nod3_needendd3
                   // ((...).-[...].-)         i(...)h...l.-j
                   ...
                   + pm1d3_needendd3[il] *
                         exp_dangle3(l, h,
                                     l + 1) * // we don't need EXPC[1] here,
                                              // because it's in pm1d3_needendd3
                         (u1_ip_jp[ip2hm1] +
                          exp_dangle5(l, h, h - 1) * u1_ip_ju[ip2hm1])
                   // (.(...).-[...].-)        i.(...)h...l.-j
                   ...
                   // case (.(..-)-[...].-(--)-) or (.(..-)-[...](--)-)
                   + EXPC[1] *
                         (pmd3_needmidd3[il] * exp_dangle3(l, h, l + 1) *
                              EXPC[1] +
                          pmd3_noneedmidd3[il]) *
                         (u1_ip_jp[ip2hm1] +
                          exp_dangle5(l, h, h - 1) * u1_ip_ju[ip2hm1])
    ...
    for (h = 0; h < seqlen; h++) {
      ...
      // for (l=seqlen-1; l > h+TURN; l--)
      {
        ...
        for (i = 0; i < h - 1; i++) {
          int il = index[i] + l - i;
          IFD { temp += (h - i - 1) * EXPC[h - i - 1] * pm[il]; }
          else {
            // the case when h-l is the first branch of the multi-loop
            temp += (h - i - 1) * EXPC[h - i - 1] *
                    (i < h - 2 ? exp_dangle5(l, h, h - 1) : 1) *
                    (pmd3_noneedmidd3[il] +
                     pmd3_needmidd3[il] * exp_dangle3(l, h, l + 1) * EXPC[1]);
            // (.-[...](---)-)    i.-h...l(---)-j
    ...
    // second, there is at least a branch to the left of k, and a branch to the
    // right Now, k is the free base inside a multi-loop, closed be i,j Removed
    // TURN on Apr 27, 2008
      ...
      // for (j=i+2*TURN+5; j < seqlen; j++)
      {
        ...
        if (upm[ij] > 0) {
          ...
          // for (k=i+TURN+3; k < j-TURN-2; k++)
          {
            ...
            IFD { temp += EXPC[1] * u1[ip1km1] * u1[kp1jm1]; }
            else {
              ...
              temp +=
                  ...
                  (exp_dangle3(i, j, i + 1) * EXPC[1] *
                       (u1_ip_ju[ip2k] + u1_iu_ju[ip2k]) // (.-(---)-.k(
                   + u1_ip_ju[ip1k]) *                   // ((---).k(
                  // the branch(es) to the right of k
                  (u1_iu_jp[kjm1] / EXPC[1] +
                   u1_iu_ju[kjm1] / EXPC[1] * exp_dangle5(i, j, j - 1));

              // second situation (---)k---
              temp +=
                  // the branch(es) to the left of k
                  (exp_dangle3(i, j, i + 1) * EXPC[1] *
                       (u1_ip_ju_jm1p[ip2k] + u1_iu_ju_jm1p[ip2k]) +
                   u1_ip_ju_jm1p[ip1k]) * // only j should be paired, not j-1
                  // the branch(es) to the right of k
                  (u1_ip_jp[kp1jm1] + u1_iu_jp[kp1jm1] +
                   (u1_ip_ju[kp1jm1] + u1_iu_ju[kp1jm1]) *
                       exp_dangle5(i, j, j - 1));
            }
          }
          GlogZ[sizeindex] +=
              p[ij] / up[ij] * EXPA * EXPB1 * exp_AUpenalty(i, j) * temp;
        }
      }
    }
    ...
    for (h = 0; h < seqlen; h++) {
      ...
      // for (l=seqlen-3; l > h+TURN; l--)
      {
        ...
        PFTYPE temp = 0;
        IFD {
          // GlogZ[sizeindex] += (j-l-1)*up[hl] * exp_AUpenalty (h,l) * EXPA *
          // EXPB2 * EXPC[j-l-1] * pm2[hj];
          for (j = l + 2; j < seqlen; j++) {
            hj = index[h] + j - h;
            temp += (j - l - 1) * EXPC[j - l - 1] * pm2[hj];
          }
          GlogZ[sizeindex] +=
              up[hl] * exp_AUpenalty(h, l) * EXPA * EXPB2 * temp;
        }
        else {
          j = l + 2;
          hj = index[h] + j - h;
          ...
          temp += (j - l - 1) * EXPC[j - l - 1] *
                  (pm2nod5_noneedmidd5[hj] +
                   pm2nod5_needmidd5[hj] * exp_dangle5(l, h, h - 1));
          // (-(---)[...].) or (-(---).[...].)
          // (-(---)-..[...].)
          ...
          for (j = l + 3; j < seqlen; j++) {
            ...
            temp += (j - l - 1) * EXPC[j - l - 1] *
                    (pm2d5_noneedmidd5[hj] +
                     pm2d5_needmidd5[hj] * exp_dangle5(l, h, h - 1));
            // (-(---)[...].-.) or (-(---).[...].-.)
            // (-(---)-..[...].-.)
          }
          GlogZ[sizeindex] += up[hl] * exp_dangle3(l, h, l + 1) *
                              exp_AUpenalty(h, l) * EXPA * EXPB2 * temp;
        }
      }
    }
  } // end if !ignore_multi
  ...
  for (i = 0; i < num_params; i++) {
    ...
    if (((GlogZexhaustive[i] != 0 || GlogZ[i] != 0) && diff > PREC) ||
      ...
      printf("%4d %30s\texhaust=%10g\tsmart=%10g\tdiff=%g\n", i,
             string_params[i], GlogZexhaustive[i], GlogZ[i], diff);
      correct = 0;
    } else if (GlogZexhaustive[i] != 0)
      printf("%4d %30s\texhaust=%10g\tsmart=%10g\t+++++++++\n", i,
             string_params[i], GlogZexhaustive[i], GlogZ[i]);
  }
  return correct;
}
...
/*
// compute_upm which has both IFD and else, just in case I messed up something
when I split them
...
j-1: l+1+TURN <= j-1 for (l=i+2; l < j-TURN-2; l++)    // case ((...)--(--)-)
    {
        ...
        IFD
            ...
            upm[ij] += up[ip1l] * exp_AUpenalty (i+1,l) *
                    (  u1_ip_jp[lp1jm1]      // [(...)(.-..)] or [(...)(.-..).]
                       + exp_dangle3 (l, i+1, l+1) * EXPC[1] *
                            ( u1_ip_jp[lp2jm1] + u1_iu_jp[lp2jm1] +     //
[(...).-(...)] or [(...).-(...).] exp_dangle5(i,j,j-1) * (u1_ip_ju[lp2jm1] +
u1_iu_ju[lp2jm1])) +    // [(...).-(...)-..]
                       + exp_dangle5(i,j,j-1) * u1_ip_ju[lp1jm1]);    //
[(...)(...)-..]
    }
