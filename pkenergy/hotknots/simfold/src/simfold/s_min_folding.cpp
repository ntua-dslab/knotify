...
// helper function, it folds sequence, returns structure and free energy
{
  ...
  for (j = 0; j < nb_nucleotides; j++) {
    ...
    // if I put this before V calculation, WM(i,j) cannot be calculated, because
    // it returns infinity
  ...
  if (cur_interval->type == LOOP) {
    ...
    if (type == STACK) {
      ...
      for (k = i + TURN + 1; k <= j - TURN - 2; k++) {
        ...
        tmp =
            VM->get_energy_WM(i + 2, k) + VM->get_energy_WM(k + 1, j - 1) +
        ...
        tmp =
            VM->get_energy_WM(i + 1, k) + VM->get_energy_WM(k + 1, j - 2) +
        ...
        tmp =
            VM->get_energy_WM(i + 2, k) + VM->get_energy_WM(k + 1, j - 2) +
            dangle_top[int_sequence[i]][int_sequence[j]][int_sequence[i + 1]] +
            dangle_bot[int_sequence[i]][int_sequence[j]][int_sequence[j - 1]] +
    ...
    for (i = 0; i <= j - TURN - 1; i++) {
      ...
      if (energy_ij < INF) {
        tmp =
            energy_ij + AU_penalty(int_sequence[i + 1], int_sequence[j]) + acc;
        tmp +=
      ...
      if (energy_ij < INF) {
        tmp =
            energy_ij + AU_penalty(int_sequence[i], int_sequence[j - 1]) + acc;
        tmp +=
      ...
      if (energy_ij < INF) {
        tmp = energy_ij + AU_penalty(int_sequence[i + 1], int_sequence[j - 1]) +
              acc;
        tmp += dangle_bot[int_sequence[j - 1]][int_sequence[i + 1]]
                         [int_sequence[i]];
        tmp += dangle_top[int_sequence[j - 1]][int_sequence[i + 1]]
                         [int_sequence[j]];
        if (tmp < min) {
          min = tmp;
          best_i = i;
    ...
    tmp = V->get_energy(i + 1, j) +
          ...
          dangle_bot[int_sequence[j]][int_sequence[i + 1]][int_sequence[i]] +
          misc.multi_helix_penalty + misc.multi_free_base_penalty;
    if (tmp < min) {
      min = tmp;
      best_row = 2;
    }
    ...
    tmp =
        V->get_energy(i + 1, j - 1) +
        AU_penalty(int_sequence[i + 1], int_sequence[j - 1]) +
        dangle_bot[int_sequence[j - 1]][int_sequence[i + 1]][int_sequence[i]] +
        dangle_top[int_sequence[j - 1]][int_sequence[i + 1]][int_sequence[j]] +
        misc.multi_helix_penalty + 2 * misc.multi_free_base_penalty;
    if (tmp < min) {
      min = tmp;
      best_row = 4;
    }
    ...
    case 1:
      ...
      break;
    case 2:
      insert_node(i + 1, j, LOOP);
      break;
    case 3:
      insert_node(i, j - 1, LOOP);
      break;
    case 4:
      insert_node(i + 1, j - 1, LOOP);
      break;
...
void s_min_folding::backtrack_restricted(seq_interval *cur_interval,
                                         str_features *fres)
// PRE:  All matrixes V, VM, WM and W have been filled
// POST: Discover the MFE path
{
  ...
  if (cur_interval->type == LOOP) {
      ...
      for (k = i + 1; k <= j - 1; k++) {
        ...
        if (fres[i + 1].pair <= -1) {
          tmp = VM->get_energy_WM(i + 2, k) + VM->get_energy_WM(k + 1, j - 1) +
                dangle_top[int_sequence[i]][int_sequence[j]]
                          [int_sequence[i + 1]] +
                misc.multi_free_base_penalty;
          if (tmp < min) {
            min = tmp;
    ...
    for (i = 0; i <= j - 1; i++) // no TURN
    {
      ...
      if (fres[i].pair <= -1) {
        energy_ij = V->get_energy(i + 1, j);
        if (energy_ij < INF) {
          tmp = energy_ij + AU_penalty(int_sequence[i + 1], int_sequence[j]) +
                acc;
          tmp +=
              dangle_bot[int_sequence[j]][int_sequence[i + 1]][int_sequence[i]];
          //                 if (fres[i+1].pair == j)
      ...
      if (fres[j].pair <= -1) {
        energy_ij = V->get_energy(i, j - 1);
        if (energy_ij < INF) {
          tmp = energy_ij + AU_penalty(int_sequence[i], int_sequence[j - 1]) +
                acc;
          tmp +=
              dangle_top[int_sequence[j - 1]][int_sequence[i]][int_sequence[j]];
          //                 if (fres[i].pair == j-1)
      ...
      if (fres[i].pair <= -1 && fres[j].pair <= -1) {
        ...
        if (energy_ij < INF) {
          tmp = energy_ij +
                AU_penalty(int_sequence[i + 1], int_sequence[j - 1]) + acc;
          tmp += dangle_bot[int_sequence[j - 1]][int_sequence[i + 1]]
                           [int_sequence[i]];
          tmp += dangle_top[int_sequence[j - 1]][int_sequence[i + 1]]
                           [int_sequence[j]];
    ...
    case 1:
      insert_node(best_i, j, LOOP);
      if (best_i - 1 >
          0) // it was TURN instead of 0  - not sure if TURN shouldn't be here
        insert_node(0, best_i - 1, FREE);
      break;
    case 2:
      insert_node(best_i + 1, j, LOOP);
    ...
    if (fres[i].pair <= -1 && fres[j].pair <= -1) {
      tmp = V->get_energy(i + 1, j - 1) +
            AU_penalty(int_sequence[i + 1], int_sequence[j - 1]) +
            dangle_bot[int_sequence[j - 1]][int_sequence[i + 1]]
                      [int_sequence[i]] +
            dangle_top[int_sequence[j - 1]][int_sequence[i + 1]]
                      [int_sequence[j]] +
            misc.multi_helix_penalty + 2 * misc.multi_free_base_penalty;
      if (tmp < min) {
        min = tmp;
...
// compute W(j)
{
  ...
  m2 = compute_W_br2(j);
  if (m1 <
      m2) // does this make sense?    || (m1 >= MAXENERGY && m2 >= MAXENERGY))
  {
    W[j] = m1;
  } else {
    W[j] = m2;
...
PARAMTYPE s_min_folding::compute_W_br2_restricted(int j, str_features *fres,
...
// May 16, 2007: this function had some bugs.
{
  ...
  // May 16, 2007: finally I think it's a good idea to include this if
  // TODO: do I need this or not??
  if ((fres[j].pair >= 0 && j > fres[j].pair) ||    // it is restricted to pair,
  i.e. MUST pair with i (fres[j-1].pair >= 0 && j-1 > fres[j-1].pair &&
  fres[j].pair <= -1))
  // not sure if I need j > fres[j].pair
  {
      if (fres[j].pair >= 0)
      {
          i = fres[j].pair;
          ...
          // May 16, 2007: added this if
          if (fres[i-1].pair <= -1 && i < j)
          {
              acc = (i-2>0) ? W[i-2]: 0;
              energy_ij = V->get_energy(i,j);
              tmp = energy_ij + AU_penalty (int_sequence[i],int_sequence[j]) +
  acc; tmp += dangle_bot [int_sequence[j]] [int_sequence[i]]
                              [int_sequence[i-1]];
              if (tmp < min)
              {
                  min = tmp;
  ...
  acc; if (tmp < min)
          ...
          // May 16, 2007: added this if
          if (fres[i-1].pair <= -1 && i < j-1)
          {
              acc = (i-2>0) ? W[i-2]: 0;
              energy_ij = V->get_energy(i,j-1);
              tmp = energy_ij + AU_penalty (int_sequence[i],int_sequence[j-1]) +
  acc; tmp += dangle_bot [int_sequence[j-1]] [int_sequence[i]]
                              [int_sequence[i-1]];
              if (tmp < min)
              {
                  min = tmp;
  ...
  */
  ...
  for (i = 0; i <= j - 1; i++) // TURN shouldn't be there
  {
    ...
    // We don't need to make sure i and j don't have to pair with something
    // else,
    ...
    // I have to condition on  fres[i].pair <= -1 to make sure that i can be
    // unpaired
    ...
    // I have to condition on  fres[j].pair <= -1 to make sure that j can be
    // unpaired
...
// compute W(j)
{
  ...
  if (must_choose_this_branch) {
    ...
    if (m1 < m2) // this is kind of stupid, dunno why it's here    || (m1 >=
                 // MAXENERGY && m2 >= MAXENERGY))
    {
      W[j] = m1;
  ...
  for (i = 0; i < nb_nucleotides; i++) {
    if (f[i].pair > i) {
      if (f[i].type == HAIRP)
        ...
        energy =
            V->get_energy(i, f[i].pair) - V->get_energy(i + 1, f[i + 1].pair);
      /*
      else if (f[i].type == INTER)
      {
          V_node = V->get_node(i, f[i].pair);
          energy = V_node->energy - V->get_energy (((internal_details
      *)V_node->details)->prime.i,
                                                   ((internal_details
      *)V_node->details)->prime.j);
      }
